...
> **Đáp án:** B
> **Giải thích:** Machine Learning nằm trong phạm vi rộng hơn của Artificial Intelligence và học quy luật từ dữ liệu.
> **Thuật ngữ:** **Artificial Intelligence**: AI — trí tuệ nhân tạo; lĩnh vực tạo hệ thống có khả năng thực hiện nhiệm vụ thường cần trí tuệ con người.; **Machine Learning**: ML — học máy; nhánh AI học quy luật từ dữ liệu thay vì chỉ dùng luật viết sẵn.
...
> **Đáp án:** C
> **Giải thích:** Slide phân biệt Discriminative AI với Generative AI: nhóm đầu chủ yếu phân loại và dự đoán.
> **Thuật ngữ:** **discriminative**: mô hình phân biệt; thường phân loại hoặc dự đoán nhãn đã định.; **generative**: mô hình tạo sinh; có khả năng tạo nội dung mới từ mẫu đã học.; **AI**: Artificial Intelligence — trí tuệ nhân tạo.
...
> **Đáp án:** D
> **Giải thích:** Generative AI sinh văn bản, hình ảnh hoặc nội dung mới thay vì chỉ chọn nhãn.
> **Thuật ngữ:** **generative**: mô hình tạo sinh; có khả năng tạo nội dung mới từ mẫu đã học.; **AI**: Artificial Intelligence — trí tuệ nhân tạo.
...
> **Đáp án:** A
> **Giải thích:** Slide định nghĩa LLM là mô hình ngôn ngữ lớn dựa trên Transformer và dữ liệu văn bản quy mô lớn.
> **Thuật ngữ:** **Transformer**: kiến trúc mạng dùng attention để xử lý quan hệ giữa các token trong chuỗi.; **LLM**: Large Language Model — mô hình ngôn ngữ lớn.
...
> **Đáp án:** C
> **Giải thích:** Self-attention cho phép mô hình cân nhắc quan hệ giữa các token trong cùng ngữ cảnh.
> **Thuật ngữ:** **self-attention**: cơ chế để mỗi token cân nhắc mức liên quan với các token khác trong cùng context.; **Transformer**: kiến trúc mạng dùng attention để xử lý quan hệ giữa các token trong chuỗi.; **token**: đơn vị văn bản mà LLM trực tiếp xử lý và thường dùng để tính chi phí.
...
> **Đáp án:** D
> **Giải thích:** Token có thể là từ, một phần từ hoặc ký hiệu; đây là đơn vị tính toán và tính phí.
> **Thuật ngữ:** **token**: đơn vị văn bản mà LLM trực tiếp xử lý và thường dùng để tính chi phí.; **LLM**: Large Language Model — mô hình ngôn ngữ lớn.
...
> **Đáp án:** A
> **Giải thích:** LLM sinh văn bản tuần tự bằng cách ước lượng xác suất cho token tiếp theo.
> **Thuật ngữ:** **next-token prediction**: dự đoán token tiếp theo dựa trên các token đã có.; **context**: bối cảnh hoặc dữ liệu nền được đưa cho model để xử lý task.; **token**: đơn vị văn bản mà LLM trực tiếp xử lý và thường dùng để tính chi phí.; **LLM**: Large Language Model — mô hình ngôn ngữ lớn.
...
> **Đáp án:** B
> **Giải thích:** Pre-training là giai đoạn mô hình đọc dữ liệu quy mô lớn để học biểu diễn ngôn ngữ và kiến thức.
> **Thuật ngữ:** **pre-training**: huấn luyện trước trên dữ liệu lớn để học ngôn ngữ và tri thức nền.; **LLM**: Large Language Model — mô hình ngôn ngữ lớn.
...
> **Đáp án:** D
> **Giải thích:** Nếu không có retrieval hoặc công cụ cập nhật, LLM không tự biết sự kiện sau thời điểm training.
> **Thuật ngữ:** **retrieval**: truy xuất; tìm các chunk liên quan từ kho dữ liệu.; **LLM**: Large Language Model — mô hình ngôn ngữ lớn.
...
> **Đáp án:** C
> **Giải thích:** Slide Token Economy nêu tổng chi phí dựa trên token đầu vào và token đầu ra.
> **Thuật ngữ:** **token**: đơn vị văn bản mà LLM trực tiếp xử lý và thường dùng để tính chi phí.; **API**: Application Programming Interface — giao diện để phần mềm gửi yêu cầu và nhận kết quả từ dịch vụ.
...
> **Đáp án:** B
> **Giải thích:** Input token nhiều hơn làm tăng chi phí và thời gian xử lý, dù số token output không đổi.
> **Thuật ngữ:** **context**: bối cảnh hoặc dữ liệu nền được đưa cho model để xử lý task.; **latency**: độ trễ; thời gian từ lúc gửi yêu cầu đến khi nhận kết quả.; **prompt**: chỉ dẫn hoặc đầu vào bằng ngôn ngữ tự nhiên gửi cho model.; **token**: đơn vị văn bản mà LLM trực tiếp xử lý và thường dùng để tính chi phí.
...
> **Đáp án:** C
> **Giải thích:** Slide khuyên bắt đầu temperature=0 cho code, phân tích hoặc tác vụ cần tính tất định hơn.
> **Thuật ngữ:** **temperature**: tham số sampling điều khiển mức đa dạng/ngẫu nhiên của output.
...
> **Đáp án:** D
> **Giải thích:** Context window là số token tối đa mô hình xử lý trong một lần gọi, gồm input và output.
> **Thuật ngữ:** **context window**: cửa sổ ngữ cảnh; tổng số token model có thể xử lý trong một lần gọi.; **context**: bối cảnh hoặc dữ liệu nền được đưa cho model để xử lý task.; **token**: đơn vị văn bản mà LLM trực tiếp xử lý và thường dùng để tính chi phí.
...
> **Đáp án:** A
> **Giải thích:** Streaming giảm cảm giác chờ bằng cách gửi chunk sớm; nó không tự giảm token hay lỗi nội dung.
> **Thuật ngữ:** **streaming**: trả kết quả từng phần ngay khi sinh, thay vì chờ toàn bộ output.; **token**: đơn vị văn bản mà LLM trực tiếp xử lý và thường dùng để tính chi phí.
...
> **Đáp án:** C
> **Giải thích:** Framework chọn model cân bằng chất lượng, latency và chi phí theo độ khó của task.
> **Thuật ngữ:** **latency**: độ trễ; thời gian từ lúc gửi yêu cầu đến khi nhận kết quả.; **task**: nhiệm vụ cụ thể mà prompt yêu cầu model hoàn thành.
...
> **Đáp án:** D
> **Giải thích:** Self-host tăng quyền kiểm soát và riêng tư nhưng đổi lại là chi phí, hạ tầng và vận hành.
> **Thuật ngữ:** **LLM**: Large Language Model — mô hình ngôn ngữ lớn.
...
> **Đáp án:** A
> **Giải thích:** Wrapper giúp chuẩn hóa provider, cấu hình và theo dõi usage để code ứng dụng gọn hơn.
> **Thuật ngữ:** **token**: đơn vị văn bản mà LLM trực tiếp xử lý và thường dùng để tính chi phí.; **API**: Application Programming Interface — giao diện để phần mềm gửi yêu cầu và nhận kết quả từ dịch vụ.
...
> **Đáp án:** B
> **Giải thích:** Các tham số sampling cao hơn thường mở rộng lựa chọn token; cần cân bằng với tính ổn định.
> **Thuật ngữ:** **temperature**: tham số sampling điều khiển mức đa dạng/ngẫu nhiên của output.; **token**: đơn vị văn bản mà LLM trực tiếp xử lý và thường dùng để tính chi phí.; **top_p**: tham số giới hạn tập token có xác suất tích lũy cao được dùng khi sampling.; **task**: nhiệm vụ cụ thể mà prompt yêu cầu model hoàn thành.
...
> **Đáp án:** D
> **Giải thích:** Day 4 nhấn mạnh token thừa vừa tốn chi phí vừa có thể làm model khó tập trung.
> **Thuật ngữ:** **context**: bối cảnh hoặc dữ liệu nền được đưa cho model để xử lý task.; **prompt**: chỉ dẫn hoặc đầu vào bằng ngôn ngữ tự nhiên gửi cho model.; **token**: đơn vị văn bản mà LLM trực tiếp xử lý và thường dùng để tính chi phí.
...
> **Đáp án:** C
> **Giải thích:** Workflow của Vibe Coding là Idea → Prompt → Code → Test → Refine, không dừng ở việc sinh code.
> **Thuật ngữ:** **workflow**: quy trình gồm các bước và bàn giao để hoàn thành một công việc.; **prompt**: chỉ dẫn hoặc đầu vào bằng ngôn ngữ tự nhiên gửi cho model.; **AI**: Artificial Intelligence — trí tuệ nhân tạo.
...
> **Đáp án:** B
> **Giải thích:** Q·K tạo attention score; sau scaling và softmax, score cao làm value tương ứng đóng góp nhiều hơn.
> **Thuật ngữ:** **self-attention**: cơ chế để mỗi token cân nhắc mức liên quan với các token khác trong cùng context.; **token**: đơn vị văn bản mà LLM trực tiếp xử lý và thường dùng để tính chi phí.
...
> **Đáp án:** C
> **Giải thích:** Scaling giúp gradient và phân phối softmax ổn định hơn khi vector có nhiều chiều.
> **Thuật ngữ:** Không có thuật ngữ tiếng Anh chuyên môn mới cần giải thích trong câu này.
...
> **Đáp án:** D
> **Giải thích:** Causal mask bảo đảm quá trình next-token prediction không nhìn trước đáp án tương lai.
> **Thuật ngữ:** **next-token prediction**: dự đoán token tiếp theo dựa trên các token đã có.; **self-attention**: cơ chế để mỗi token cân nhắc mức liên quan với các token khác trong cùng context.; **Transformer**: kiến trúc mạng dùng attention để xử lý quan hệ giữa các token trong chuỗi.; **token**: đơn vị văn bản mà LLM trực tiếp xử lý và thường dùng để tính chi phí.
...
> **Đáp án:** A
> **Giải thích:** Tokenization không đồng nhất giữa ngôn ngữ; số token, không phải số từ trực tiếp, quyết định cost.
> **Thuật ngữ:** **prompt**: chỉ dẫn hoặc đầu vào bằng ngôn ngữ tự nhiên gửi cho model.; **token**: đơn vị văn bản mà LLM trực tiếp xử lý và thường dùng để tính chi phí.
...
> **Đáp án:** B
> **Giải thích:** Day 8 đưa ví dụ phân bổ 20/60/20 để tránh nhồi context đến sát giới hạn.
> **Thuật ngữ:** **system prompt**: chỉ dẫn nền do ứng dụng đặt để định nghĩa vai trò, luật và capability của model.; **context**: bối cảnh hoặc dữ liệu nền được đưa cho model để xử lý task.; **prompt**: chỉ dẫn hoặc đầu vào bằng ngôn ngữ tự nhiên gửi cho model.
...
> **Đáp án:** C
> **Giải thích:** Chi phí phải nhân số lượt với token từng loại rồi áp đơn giá input và output riêng.
> **Thuật ngữ:** **chatbot**: hệ thống hội thoại thường trả lời bằng văn bản theo context của lượt chat.; **token**: đơn vị văn bản mà LLM trực tiếp xử lý và thường dùng để tính chi phí.
...
> **Đáp án:** D
> **Giải thích:** Tính ổn định của sampling không tạo thêm evidence hoặc loại bỏ lỗi kiến trúc.
> **Thuật ngữ:** **temperature**: tham số sampling điều khiển mức đa dạng/ngẫu nhiên của output.; **retrieval**: truy xuất; tìm các chunk liên quan từ kho dữ liệu.; **prompt**: chỉ dẫn hoặc đầu vào bằng ngôn ngữ tự nhiên gửi cho model.
...
> **Đáp án:** A
> **Giải thích:** Framework chọn model theo task, ưu tiên cost/latency khi chất lượng model nhỏ đã đủ.
> **Thuật ngữ:** **reasoning**: suy luận; phân tích mục tiêu và chọn bước tiếp theo.; **latency**: độ trễ; thời gian từ lúc gửi yêu cầu đến khi nhận kết quả.; **task**: nhiệm vụ cụ thể mà prompt yêu cầu model hoàn thành.
...
> **Đáp án:** B
> **Giải thích:** Day 2 nhấn mạnh không giải quyết vội giải pháp được yêu cầu; cần tìm đúng vấn đề trước.
...
> **Đáp án:** C
> **Giải thích:** 'Chatbot' có thể che nhiều bài toán khác nhau như FAQ, routing, QA hoặc phân tích cảm xúc.
> **Thuật ngữ:** **chatbot**: hệ thống hội thoại thường trả lời bằng văn bản theo context của lượt chat.
...
> **Đáp án:** D
> **Giải thích:** Hai hình thoi lần lượt giúp tìm đúng vấn đề và tìm đúng giải pháp.
> **Thuật ngữ:** Không có thuật ngữ tiếng Anh chuyên môn mới cần giải thích trong câu này.
...
> **Đáp án:** A
> **Giải thích:** HCD bắt đầu từ việc quan sát đối tượng mục tiêu và workflow thực tế của họ.
> **Thuật ngữ:** **observation**: quan sát; kết quả tool hoặc môi trường trả lại cho agent.; **workflow**: quy trình gồm các bước và bàn giao để hoàn thành một công việc.
...
> **Đáp án:** C
> **Giải thích:** Không có baseline thì khó biết AI cải thiện điều gì so với rule hoặc quy trình thủ công.
> **Thuật ngữ:** **baseline**: mốc chuẩn dùng để so sánh với cách làm hiện tại.; **AI**: Artificial Intelligence — trí tuệ nhân tạo.
...
> **Đáp án:** D
> **Giải thích:** Problem statement tốt nối vấn đề kinh doanh với tiêu chí eval và boundary kỹ thuật.
> **Thuật ngữ:** **problem statement**: mô tả bài toán rõ actor, pain point, metric, boundary và cách đánh giá.; **eval**: viết tắt của evaluation — đánh giá chất lượng AI.
...
> **Đáp án:** A
> **Giải thích:** No eval path khiến nhóm không có cách đo chất lượng hoặc quyết định deploy.
> **Thuật ngữ:** **eval**: viết tắt của evaluation — đánh giá chất lượng AI.
...
> **Đáp án:** B
> **Giải thích:** Day 2 khuyên chỉ tăng độ phức tạp khi giá trị tăng lớn hơn chi phí và rủi ro.
> **Thuật ngữ:** **workflow**: quy trình gồm các bước và bàn giao để hoàn thành một công việc.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.
...
> **Đáp án:** D
> **Giải thích:** Feedback từ production có thể quay lại làm dữ liệu và cải thiện hệ thống qua nhiều vòng.
> **Thuật ngữ:** **AI**: Artificial Intelligence — trí tuệ nhân tạo.
...
> **Đáp án:** C
> **Giải thích:** Go/No-Go cần dựa vào problem, metric, baseline và rủi ro chứ không chỉ dựa vào demo.
> **Thuật ngữ:** **baseline**: mốc chuẩn dùng để so sánh với cách làm hiện tại.; **eval**: viết tắt của evaluation — đánh giá chất lượng AI.
...
> **Đáp án:** B
> **Giải thích:** Prototype giúp học nhanh từ giả thuyết rẻ nhất trước khi build đầy đủ.
> **Thuật ngữ:** **prototype**: bản mẫu nhanh để kiểm tra giả thuyết trước khi build đầy đủ.; **AI**: Artificial Intelligence — trí tuệ nhân tạo.
...
> **Đáp án:** C
> **Giải thích:** Low-fidelity phù hợp để kiểm tra workflow; high-fidelity mới gần trải nghiệm thật hơn.
> **Thuật ngữ:** **prototype**: bản mẫu nhanh để kiểm tra giả thuyết trước khi build đầy đủ.; **workflow**: quy trình gồm các bước và bàn giao để hoàn thành một công việc.
...
> **Đáp án:** D
> **Giải thích:** Wizard of Oz kiểm tra nhu cầu bằng trải nghiệm giống AI nhưng phần xử lý có thể thủ công.
> **Thuật ngữ:** **MVP**: Minimum Viable Product — sản phẩm tối thiểu nhưng dùng được để kiểm chứng giả thuyết.; **AI**: Artificial Intelligence — trí tuệ nhân tạo.
...
> **Đáp án:** A
> **Giải thích:** Day 5 nhắc problem statement nên nói về vấn đề của user, không giả định AI là đáp án.
> **Thuật ngữ:** **problem statement**: mô tả bài toán rõ actor, pain point, metric, boundary và cách đánh giá.; **AI**: Artificial Intelligence — trí tuệ nhân tạo.
...
> **Đáp án:** C
> **Giải thích:** Non-goals giúp kiểm soát kỳ vọng và tránh mở rộng scope ngoài mục tiêu.
> **Thuật ngữ:** Không có thuật ngữ tiếng Anh chuyên môn mới cần giải thích trong câu này.
...
> **Đáp án:** D
> **Giải thích:** Mức agency nên chọn theo độ chắc chắn và cost-of-error của hành động.
> **Thuật ngữ:** **augmentation**: AI hỗ trợ con người, còn người dùng giữ quyền quyết định cuối.; **automation**: AI tự thực hiện phần lớn hoặc toàn bộ workflow theo quy tắc đã định.
...
> **Đáp án:** A
> **Giải thích:** Với hành động khó hoàn tác, friction của phê duyệt là cơ chế kiểm soát hữu ích.
> **Thuật ngữ:** Không có thuật ngữ tiếng Anh chuyên môn mới cần giải thích trong câu này.
...
> **Đáp án:** B
> **Giải thích:** Precision quan tâm trong các kết quả được dự đoán là dương tính, nên nhạy với false positive.
> **Thuật ngữ:** **false positive**: báo nhầm một trường hợp tốt/thấp rủi ro thành trường hợp dương tính.; **precision**: tỷ lệ kết quả được dự đoán dương tính mà thực sự đúng.; **recall**: tỷ lệ các trường hợp đúng mà hệ thống tìm được.
...
> **Đáp án:** D
> **Giải thích:** Recall đo mức bao phủ các trường hợp đúng, nên cần giảm bỏ sót.
> **Thuật ngữ:** **false negative**: bỏ sót một trường hợp thực sự cần được phát hiện.; **precision**: tỷ lệ kết quả được dự đoán dương tính mà thực sự đúng.; **recall**: tỷ lệ các trường hợp đúng mà hệ thống tìm được.
...
> **Đáp án:** C
> **Giải thích:** Demo mượt vài câu không thay thế được bộ case đại diện và quy trình đánh giá lặp lại.
> **Thuật ngữ:** **eval**: viết tắt của evaluation — đánh giá chất lượng AI.; **AI**: Artificial Intelligence — trí tuệ nhân tạo.
...
> **Đáp án:** B
> **Giải thích:** AI phù hợp với tác vụ có biến thể vừa phải; chưa cần tăng lên agent nếu workflow có thể kiểm soát.
> **Thuật ngữ:** **workflow**: quy trình gồm các bước và bàn giao để hoàn thành một công việc.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.; **LLM**: Large Language Model — mô hình ngôn ngữ lớn.; **AI**: Artificial Intelligence — trí tuệ nhân tạo.
...
> **Đáp án:** C
> **Giải thích:** Discovery interview cần làm rõ vấn đề, tần suất, workflow và cost trước khi chọn kiến trúc.
> **Thuật ngữ:** **stakeholder**: bên liên quan hoặc người có lợi ích/quyền quyết định trong bài toán.; **workflow**: quy trình gồm các bước và bàn giao để hoàn thành một công việc.; **actor**: tác nhân trực tiếp thực hiện hoặc chịu ảnh hưởng bởi workflow.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.
...
> **Đáp án:** D
> **Giải thích:** Baseline là điểm tham chiếu để đo giá trị tăng thêm của AI.
> **Thuật ngữ:** **baseline**: mốc chuẩn dùng để so sánh với cách làm hiện tại.; **AI**: Artificial Intelligence — trí tuệ nhân tạo.
...
> **Đáp án:** A
> **Giải thích:** Prompt chaining tách task tuần tự và có thể kiểm tra output trước khi đi tiếp.
> **Thuật ngữ:** **workflow**: quy trình gồm các bước và bàn giao để hoàn thành một công việc.; **prompt**: chỉ dẫn hoặc đầu vào bằng ngôn ngữ tự nhiên gửi cho model.; **task**: nhiệm vụ cụ thể mà prompt yêu cầu model hoàn thành.; **LLM**: Large Language Model — mô hình ngôn ngữ lớn.
...
> **Đáp án:** C
> **Giải thích:** Routing giảm prompt phức tạp bằng cách phân luồng theo loại yêu cầu.
> **Thuật ngữ:** **prompt**: chỉ dẫn hoặc đầu vào bằng ngôn ngữ tự nhiên gửi cho model.; **worker**: agent chuyên trách một năng lực hẹp như retrieve, gọi tool hoặc summarize.
...
> **Đáp án:** D
> **Giải thích:** Pattern này có một bước tạo và một bước đánh giá/cải thiện theo tiêu chí đã định.
> **Thuật ngữ:** **workflow**: quy trình gồm các bước và bàn giao để hoàn thành một công việc.
...
> **Đáp án:** B
> **Giải thích:** No eval path là anti-pattern vì nhóm không biết hệ thống đạt mức nào và có hồi quy hay không.
> **Thuật ngữ:** **eval**: viết tắt của evaluation — đánh giá chất lượng AI.
...
> **Đáp án:** D
> **Giải thích:** North Star gắn với outcome quan trọng nhất chứ không chỉ là metric kỹ thuật tiện đo.
> **Thuật ngữ:** **North Star metric**: chỉ số trung tâm gắn với outcome quan trọng nhất của sản phẩm.
...
> **Đáp án:** C
> **Giải thích:** Nhiều bước, quyết định động và tool use là các tín hiệu của Agentic Fit cao.
> **Thuật ngữ:** **workflow**: quy trình gồm các bước và bàn giao để hoàn thành một công việc.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.
...
> **Đáp án:** A
> **Giải thích:** Gate criteria yêu cầu bài toán có thể đo, có ranh giới và có đường triển khai khả thi.
> **Thuật ngữ:** **eval**: viết tắt của evaluation — đánh giá chất lượng AI.
...
> **Đáp án:** B
> **Giải thích:** Người dùng vẫn duyệt và chỉnh kết quả; mức agency thấp hơn automation hoàn toàn.
> **Thuật ngữ:** **augmentation**: AI hỗ trợ con người, còn người dùng giữ quyền quyết định cuối.; **automation**: AI tự thực hiện phần lớn hoặc toàn bộ workflow theo quy tắc đã định.
...
> **Đáp án:** C
> **Giải thích:** Hệ thống báo nhầm dương tính là false positive; nếu cost của nó cao thì ưu precision.
> **Thuật ngữ:** **false positive**: báo nhầm một trường hợp tốt/thấp rủi ro thành trường hợp dương tính.; **precision**: tỷ lệ kết quả được dự đoán dương tính mà thực sự đúng.; **recall**: tỷ lệ các trường hợp đúng mà hệ thống tìm được.
...
> **Đáp án:** D
> **Giải thích:** Người dùng tin đúng mức khi biết khả năng, lý do và quyền can thiệp của hệ thống.
> **Thuật ngữ:** Không có thuật ngữ tiếng Anh chuyên môn mới cần giải thích trong câu này.
...
> **Đáp án:** A
> **Giải thích:** Người dùng cần biết nên tin và kiểm tra thế nào hơn là một số phần trăm thiếu ngữ cảnh.
> **Thuật ngữ:** **confidence**: mức độ tự tin/độ chắc chắn mà hệ thống gán cho output hoặc evidence.; **UX**: User Experience — trải nghiệm người dùng khi tương tác với sản phẩm.
...
> **Đáp án:** B
> **Giải thích:** Graceful fallback giữ tính trung thực và giúp user bổ sung thông tin cần thiết.
> **Thuật ngữ:** **AI**: Artificial Intelligence — trí tuệ nhân tạo.
...
> **Đáp án:** C
> **Giải thích:** Nondeterminism là mặc định của hệ thống sinh; acceptance criteria cần phản ánh mức biến thiên.
> **Thuật ngữ:** Không có thuật ngữ tiếng Anh chuyên môn mới cần giải thích trong câu này.
...
> **Đáp án:** D
> **Giải thích:** Eval là chu trình, không phải một lần chấm demo.
> **Thuật ngữ:** **eval**: viết tắt của evaluation — đánh giá chất lượng AI.; **AI**: Artificial Intelligence — trí tuệ nhân tạo.
...
> **Đáp án:** A
> **Giải thích:** UI cần cho user biết khả năng và giới hạn trước khi họ giao việc cho AI.
> **Thuật ngữ:** **AI**: Artificial Intelligence — trí tuệ nhân tạo.
...
> **Đáp án:** C
> **Giải thích:** MVP kiểu Wizard of Oz kiểm tra nhu cầu và hành vi trước khi đầu tư kỹ thuật lớn.
> **Thuật ngữ:** **MVP**: Minimum Viable Product — sản phẩm tối thiểu nhưng dùng được để kiểm chứng giả thuyết.
...
> **Đáp án:** D
> **Giải thích:** Non-goal là hàng rào sản phẩm giúp team biết điều gì chưa nằm trong phiên bản này.
> **Thuật ngữ:** Không có thuật ngữ tiếng Anh chuyên môn mới cần giải thích trong câu này.
...
> **Đáp án:** B
> **Giải thích:** Problem statement cần actor, pain, tần suất và outcome; không lấy tên giải pháp làm vấn đề.
> **Thuật ngữ:** **problem statement**: mô tả bài toán rõ actor, pain point, metric, boundary và cách đánh giá.; **stakeholder**: bên liên quan hoặc người có lợi ích/quyền quyết định trong bài toán.; **actor**: tác nhân trực tiếp thực hiện hoặc chịu ảnh hưởng bởi workflow.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.
...
> **Đáp án:** C
> **Giải thích:** North Star nên nối với outcome như thời gian tiết kiệm, resolution hoặc conversion, không chỉ volume output.
> **Thuật ngữ:** **North Star metric**: chỉ số trung tâm gắn với outcome quan trọng nhất của sản phẩm.
...
> **Đáp án:** D
> **Giải thích:** Input ổn định, flow cố định và cost-of-error cao là tín hiệu không nên tăng độ tự chủ.
> **Thuật ngữ:** **workflow**: quy trình gồm các bước và bàn giao để hoàn thành một công việc.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.
...
> **Đáp án:** A
> **Giải thích:** Chaining phù hợp khi chuỗi bước tương đối ổn định và cần gate giữa các bước.
> **Thuật ngữ:** **workflow**: quy trình gồm các bước và bàn giao để hoàn thành một công việc.; **prompt**: chỉ dẫn hoặc đầu vào bằng ngôn ngữ tự nhiên gửi cho model.
...
> **Đáp án:** B
> **Giải thích:** Orchestrator-workers cho phép phân việc động; routing chỉ cần chọn nhánh đã biết.
> **Thuật ngữ:** **worker**: agent chuyên trách một năng lực hẹp như retrieve, gọi tool hoặc summarize.; **task**: nhiệm vụ cụ thể mà prompt yêu cầu model hoàn thành.
...
> **Đáp án:** C
> **Giải thích:** Discovery/feasibility phải kiểm tra nguồn, quyền, chất lượng và độ mới trước khi viết nhiều code.
> **Thuật ngữ:** **freshness**: độ mới của dữ liệu so với thời điểm user cần sử dụng.
...
> **Đáp án:** D
> **Giải thích:** Eval kỹ thuật không tự chứng minh giá trị sản phẩm; cần nối với outcome của actor.
> **Thuật ngữ:** **workflow**: quy trình gồm các bước và bàn giao để hoàn thành một công việc.; **metrics**: số đo định lượng theo thời gian như latency, error rate, token hoặc cost.; **actor**: tác nhân trực tiếp thực hiện hoặc chịu ảnh hưởng bởi workflow.; **eval**: viết tắt của evaluation — đánh giá chất lượng AI.
...
> **Đáp án:** A
> **Giải thích:** Why Johnny Can't Prompt nhấn mạnh user cần biết khả năng và cách tiếp cận task.
> **Thuật ngữ:** **prompt**: chỉ dẫn hoặc đầu vào bằng ngôn ngữ tự nhiên gửi cho model.; **task**: nhiệm vụ cụ thể mà prompt yêu cầu model hoàn thành.; **AI**: Artificial Intelligence — trí tuệ nhân tạo.
...
> **Đáp án:** B
> **Giải thích:** Tác động phải tính cả thời gian sửa, độ chính xác và rủi ro chứ không chỉ thời gian sinh output.
> **Thuật ngữ:** **baseline**: mốc chuẩn dùng để so sánh với cách làm hiện tại.; **AI**: Artificial Intelligence — trí tuệ nhân tạo.
...
> **Đáp án:** D
> **Giải thích:** Day 5 nhắc không chỉ định nghĩa đoạn AI; cần biết user đi từ input đến kết quả thế nào.
> **Thuật ngữ:** **AI**: Artificial Intelligence — trí tuệ nhân tạo.
...
> **Đáp án:** B
> **Giải thích:** Sai recipient là side effect khó hoàn tác; augmentation và approval giảm cost-of-error.
> **Thuật ngữ:** **augmentation**: AI hỗ trợ con người, còn người dùng giữ quyền quyết định cuối.; **side effect**: tác động bên ngoài như gửi email, xóa dữ liệu, trừ tiền hoặc đặt vé.; **AI**: Artificial Intelligence — trí tuệ nhân tạo.
...
> **Đáp án:** C
> **Giải thích:** Khi FN gây hại lớn, cần tối đa hóa coverage và có thể dùng human review cho phần nghi ngờ.
> **Thuật ngữ:** **false negative**: bỏ sót một trường hợp thực sự cần được phát hiện.; **precision**: tỷ lệ kết quả được dự đoán dương tính mà thực sự đúng.; **recall**: tỷ lệ các trường hợp đúng mà hệ thống tìm được.
...
> **Đáp án:** D
> **Giải thích:** AI có nhiều câu trả lời hợp lệ; acceptance cần đo mức đạt, factuality, format và failure handling.
> **Thuật ngữ:** **AI**: Artificial Intelligence — trí tuệ nhân tạo.
...
> **Đáp án:** A
> **Giải thích:** Trust giảm khi UI hứa vượt khả năng; cần nói rõ phạm vi, thời điểm dữ liệu và cách AI có thể sai.
> **Thuật ngữ:** **freshness**: độ mới của dữ liệu so với thời điểm user cần sử dụng.; **AI**: Artificial Intelligence — trí tuệ nhân tạo.
...
> **Đáp án:** B
> **Giải thích:** Control, preview và rollback cho phép dùng AI trong vùng rủi ro vừa phải.
> **Thuật ngữ:** **confidence**: mức độ tự tin/độ chắc chắn mà hệ thống gán cho output hoặc evidence.; **AI**: Artificial Intelligence — trí tuệ nhân tạo.; **UX**: User Experience — trải nghiệm người dùng khi tương tác với sản phẩm.
...
> **Đáp án:** C
> **Giải thích:** Output sinh cần chấm theo mức độ, relevance, faithfulness hoặc rubric thay vì một chuỗi duy nhất.
> **Thuật ngữ:** **faithfulness**: mức độ câu trả lời bám trung thực vào context được truy xuất.; **eval**: viết tắt của evaluation — đánh giá chất lượng AI.; **AI**: Artificial Intelligence — trí tuệ nhân tạo.
...
> **Đáp án:** D
> **Giải thích:** Fallback là quyết định sản phẩm, không chỉ là exception kỹ thuật.
> **Thuật ngữ:** Không có thuật ngữ tiếng Anh chuyên môn mới cần giải thích trong câu này.
...
> **Đáp án:** A
> **Giải thích:** Build rẻ trước để giảm rủi ro học sai bài toán.
> **Thuật ngữ:** **prototype**: bản mẫu nhanh để kiểm tra giả thuyết trước khi build đầy đủ.; **workflow**: quy trình gồm các bước và bàn giao để hoàn thành một công việc.; **MVP**: Minimum Viable Product — sản phẩm tối thiểu nhưng dùng được để kiểm chứng giả thuyết.; **AI**: Artificial Intelligence — trí tuệ nhân tạo.
...
> **Đáp án:** B
> **Giải thích:** AI-specific requirements phải nêu input/context và privacy controls, không chỉ mô tả nút bấm.
> **Thuật ngữ:** **context**: bối cảnh hoặc dữ liệu nền được đưa cho model để xử lý task.; **PII**: Personally Identifiable Information — dữ liệu có thể nhận diện một cá nhân.; **AI**: Artificial Intelligence — trí tuệ nhân tạo.
...
> **Đáp án:** C
> **Giải thích:** Các tầng quality khác nhau có thể không đồng thuận; cần xem case và outcome thay vì tin một số.
> **Thuật ngữ:** **metrics**: số đo định lượng theo thời gian như latency, error rate, token hoặc cost.; **LLM**: Large Language Model — mô hình ngôn ngữ lớn.; **AI**: Artificial Intelligence — trí tuệ nhân tạo.
...
> **Đáp án:** B
> **Giải thích:** Day 4 dùng khung Role / Task / Context / Format để biến ý định thành chỉ dẫn rõ.
> **Thuật ngữ:** **context**: bối cảnh hoặc dữ liệu nền được đưa cho model để xử lý task.; **prompt**: chỉ dẫn hoặc đầu vào bằng ngôn ngữ tự nhiên gửi cho model.; **role**: vai trò model cần đảm nhận trong prompt.; **task**: nhiệm vụ cụ thể mà prompt yêu cầu model hoàn thành.
...
> **Đáp án:** C
> **Giải thích:** Zero-shot không cung cấp example; few-shot mới dùng các ví dụ minh họa.
> **Thuật ngữ:** **zero-shot**: thực hiện task mà không được cung cấp ví dụ mẫu.; **few-shot**: cung cấp một vài ví dụ để model bắt chước cách xử lý hoặc format.; **task**: nhiệm vụ cụ thể mà prompt yêu cầu model hoàn thành.
...
> **Đáp án:** D
> **Giải thích:** Ví dụ mẫu giúp model bắt chước cách trả lời và định dạng mong muốn.
> **Thuật ngữ:** **few-shot**: cung cấp một vài ví dụ để model bắt chước cách xử lý hoặc format.
...
> **Đáp án:** A
> **Giải thích:** CoT được giới thiệu cho các bài toán có reasoning nhiều bước; không phải task nào cũng cần.
> **Thuật ngữ:** **chain-of-thought**: suy luận theo chuỗi bước; dùng để hỗ trợ task cần nhiều bước reasoning.; **reasoning**: suy luận; phân tích mục tiêu và chọn bước tiếp theo.; **task**: nhiệm vụ cụ thể mà prompt yêu cầu model hoàn thành.
...
> **Đáp án:** C
> **Giải thích:** System message là hợp đồng nền tảng định hướng hành vi của agent.
> **Thuật ngữ:** **system prompt**: chỉ dẫn nền do ứng dụng đặt để định nghĩa vai trò, luật và capability của model.; **prompt**: chỉ dẫn hoặc đầu vào bằng ngôn ngữ tự nhiên gửi cho model.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.
...
> **Đáp án:** D
> **Giải thích:** Cấu trúc hóa context làm giảm context bleed, tức nhầm dữ liệu thành mệnh lệnh.
> **Thuật ngữ:** **context bleed**: lẫn ngữ cảnh; model nhầm dữ liệu không tin cậy thành instruction cần tuân theo.; **delimiter**: dấu phân cách dùng để tách instruction khỏi dữ liệu đầu vào.; **context**: bối cảnh hoặc dữ liệu nền được đưa cho model để xử lý task.
...
> **Đáp án:** A
> **Giải thích:** Dữ liệu user, API hoặc tài liệu có thể chứa câu lệnh; delimiters giúp phân biệt chúng.
> **Thuật ngữ:** **context bleed**: lẫn ngữ cảnh; model nhầm dữ liệu không tin cậy thành instruction cần tuân theo.; **context**: bối cảnh hoặc dữ liệu nền được đưa cho model để xử lý task.; **API**: Application Programming Interface — giao diện để phần mềm gửi yêu cầu và nhận kết quả từ dịch vụ.
...
> **Đáp án:** B
> **Giải thích:** Mô tả tool là một phần prompt để model chọn đúng hàm và tham số.
> **Thuật ngữ:** **prompt**: chỉ dẫn hoặc đầu vào bằng ngôn ngữ tự nhiên gửi cho model.
...
> **Đáp án:** D
> **Giải thích:** Required giúp ngăn việc bỏ trống field cần thiết như city hoặc date.
> **Thuật ngữ:** **JSON schema**: schema JSON; mô tả cấu trúc, kiểu dữ liệu và field hợp lệ của JSON.; **required**: field bắt buộc phải có trong object hoặc tool arguments.
...
> **Đáp án:** C
> **Giải thích:** Phụ thuộc dữ liệu yêu cầu chained calls để truyền kết quả chính xác sang bước sau.
> **Thuật ngữ:** Không có thuật ngữ tiếng Anh chuyên môn mới cần giải thích trong câu này.
...
> **Đáp án:** B
> **Giải thích:** Delimiter và instruction hierarchy giúp model phân biệt dữ liệu được trích xuất với luật điều khiển.
> **Thuật ngữ:** **system prompt**: chỉ dẫn nền do ứng dụng đặt để định nghĩa vai trò, luật và capability của model.; **context bleed**: lẫn ngữ cảnh; model nhầm dữ liệu không tin cậy thành instruction cần tuân theo.; **delimiter**: dấu phân cách dùng để tách instruction khỏi dữ liệu đầu vào.; **context**: bối cảnh hoặc dữ liệu nền được đưa cho model để xử lý task.; **prompt**: chỉ dẫn hoặc đầu vào bằng ngôn ngữ tự nhiên gửi cho model.
...
> **Đáp án:** C
> **Giải thích:** Ví dụ thiếu dữ liệu, mơ hồ hoặc jailbreak giúp kiểm thử boundary của prompt.
> **Thuật ngữ:** **few-shot**: cung cấp một vài ví dụ để model bắt chước cách xử lý hoặc format.; **prompt**: chỉ dẫn hoặc đầu vào bằng ngôn ngữ tự nhiên gửi cho model.
...
> **Đáp án:** D
> **Giải thích:** Output contract rõ làm giảm format hallucination và giúp parser xử lý nhất quán hơn.
> **Thuật ngữ:** **output contract**: hợp đồng đầu ra; quy định format, field, giới hạn và điều kiện trả kết quả.; **JSON schema**: schema JSON; mô tả cấu trúc, kiểu dữ liệu và field hợp lệ của JSON.; **required**: field bắt buộc phải có trong object hoặc tool arguments.
...
> **Đáp án:** A
> **Giải thích:** Model có thể bị ảnh hưởng bởi ví dụ gần cuối; cần tránh thứ tự gây lệch nhãn.
> **Thuật ngữ:** **few-shot**: cung cấp một vài ví dụ để model bắt chước cách xử lý hoặc format.
...
> **Đáp án:** B
> **Giải thích:** Model thường chú ý tốt hơn ở đầu/cuối; compression và reordering giúp tận dụng context.
> **Thuật ngữ:** **context**: bối cảnh hoặc dữ liệu nền được đưa cho model để xử lý task.
...
> **Đáp án:** C
> **Giải thích:** Template có placeholder cho date, state hoặc service status giúp prompt thích ứng nhưng vẫn có cấu trúc.
> **Thuật ngữ:** **dynamic prompt**: prompt được dựng theo trạng thái, ngày giờ, quyền hoặc availability hiện tại.; **system prompt**: chỉ dẫn nền do ứng dụng đặt để định nghĩa vai trò, luật và capability của model.; **prompt**: chỉ dẫn hoặc đầu vào bằng ngôn ngữ tự nhiên gửi cho model.
...
> **Đáp án:** D
> **Giải thích:** Nên nêu điều kiện thiếu dữ liệu, câu trả lời fallback và escalation thay vì chỉ cấm đoán.
> **Thuật ngữ:** Không có thuật ngữ tiếng Anh chuyên môn mới cần giải thích trong câu này.
...
> **Đáp án:** A
> **Giải thích:** Enum thu hẹp không gian lựa chọn và giảm tham số sai.
> **Thuật ngữ:** **enum**: tập hữu hạn các giá trị được phép chọn.
...
> **Đáp án:** B
> **Giải thích:** Slide khuyên giữ khoảng 5–7 tham số hoặc ít hơn nếu có thể để contract dễ hiểu.
> **Thuật ngữ:** Không có thuật ngữ tiếng Anh chuyên môn mới cần giải thích trong câu này.
...
> **Đáp án:** D
> **Giải thích:** Báo lỗi có cấu trúc cho LLM tạo cơ hội sửa, nhưng vẫn cần retry limit và validation.
> **Thuật ngữ:** **LLM**: Large Language Model — mô hình ngôn ngữ lớn.
...
> **Đáp án:** B
> **Giải thích:** Prompt production-grade cần tránh mâu thuẫn hoặc nêu ưu tiên rõ khi hai yêu cầu xung đột.
> **Thuật ngữ:** **output contract**: hợp đồng đầu ra; quy định format, field, giới hạn và điều kiện trả kết quả.; **system prompt**: chỉ dẫn nền do ứng dụng đặt để định nghĩa vai trò, luật và capability của model.; **prompt**: chỉ dẫn hoặc đầu vào bằng ngôn ngữ tự nhiên gửi cho model.
...
> **Đáp án:** D
> **Giải thích:** Recency và lost-in-the-middle khiến vị trí và compression quan trọng không kém tổng dung lượng.
> **Thuật ngữ:** **context**: bối cảnh hoặc dữ liệu nền được đưa cho model để xử lý task.; **prompt**: chỉ dẫn hoặc đầu vào bằng ngôn ngữ tự nhiên gửi cho model.
...
> **Đáp án:** A
> **Giải thích:** Order bias yêu cầu kiểm tra ảnh hưởng vị trí, không dồn một loại nhãn ở cuối chuỗi.
> **Thuật ngữ:** **few-shot**: cung cấp một vài ví dụ để model bắt chước cách xử lý hoặc format.
...
> **Đáp án:** B
> **Giải thích:** Schema cần ràng buộc input; nếu chỉ có một tập mã hợp lệ thì enum giúp giảm lỗi.
> **Thuật ngữ:** **enum**: tập hữu hạn các giá trị được phép chọn.
...
> **Đáp án:** C
> **Giải thích:** Contract nhỏ, rõ giúp model chọn và điền arguments chính xác hơn.
> **Thuật ngữ:** Không có thuật ngữ tiếng Anh chuyên môn mới cần giải thích trong câu này.
...
> **Đáp án:** A
> **Giải thích:** Append-only giúp graph giữ diễn tiến; node vẫn có thể cập nhật field khác theo schema.
> **Thuật ngữ:** **LangGraph**: framework biểu diễn workflow agent bằng state, node, edge và conditional routing.; **context**: bối cảnh hoặc dữ liệu nền được đưa cho model để xử lý task.
...
> **Đáp án:** B
> **Giải thích:** Router/should_continue là hàm đọc state để quyết định vòng tiếp hoặc final answer.
> **Thuật ngữ:** **LangGraph**: framework biểu diễn workflow agent bằng state, node, edge và conditional routing.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.
...
> **Đáp án:** D
> **Giải thích:** Schema giúp model, nhưng runtime validation bảo vệ hệ thống trước input sai hoặc malicious.
> **Thuật ngữ:** Không có thuật ngữ tiếng Anh chuyên môn mới cần giải thích trong câu này.
...
> **Đáp án:** B
> **Giải thích:** Rule-based bot hoạt động theo luồng và điều kiện do con người viết sẵn.
> **Thuật ngữ:** **rule-based bot**: bot theo luật; phản hồi bằng các điều kiện if/else được định trước.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.
...
> **Đáp án:** C
> **Giải thích:** Agent kết hợp reasoning, tools, state và vòng lặp; một LLM call đơn lẻ chưa đủ là agent.
> **Thuật ngữ:** **reasoning**: suy luận; phân tích mục tiêu và chọn bước tiếp theo.; **chatbot**: hệ thống hội thoại thường trả lời bằng văn bản theo context của lượt chat.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.; **LLM**: Large Language Model — mô hình ngôn ngữ lớn.
...
> **Đáp án:** D
> **Giải thích:** Slide kiến trúc agent nêu goal, reasoning, tools và memory/state là các khối bổ sung.
> **Thuật ngữ:** **reasoning**: suy luận; phân tích mục tiêu và chọn bước tiếp theo.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.; **LLM**: Large Language Model — mô hình ngôn ngữ lớn.
...
> **Đáp án:** A
> **Giải thích:** ReAct kết hợp suy luận theo bước với hành động gọi công cụ.
> **Thuật ngữ:** **reasoning**: suy luận; phân tích mục tiêu và chọn bước tiếp theo.; **acting**: hành động; bước agent thực thi tool hoặc tác động lên môi trường.; **ReAct**: Reasoning + Acting — suy luận kết hợp hành động theo vòng Thought → Action → Observation.
...
> **Đáp án:** D
> **Giải thích:** Tool là 'tay chân' giúp agent lấy dữ liệu hoặc thực hiện hành động bên ngoài LLM.
> **Thuật ngữ:** **tool calling**: gọi công cụ; cho phép model yêu cầu ứng dụng thực thi API, database hoặc hàm bên ngoài.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.; **LLM**: Large Language Model — mô hình ngôn ngữ lớn.; **API**: Application Programming Interface — giao diện để phần mềm gửi yêu cầu và nhận kết quả từ dịch vụ.
...
> **Đáp án:** A
> **Giải thích:** Short-term memory giữ lịch sử gần đây và thông tin cần cho phiên hiện tại.
> **Thuật ngữ:** **short-term memory**: bộ nhớ ngắn hạn; lịch sử/context của task hiện tại.; **context window**: cửa sổ ngữ cảnh; tổng số token model có thể xử lý trong một lần gọi.; **context**: bối cảnh hoặc dữ liệu nền được đưa cho model để xử lý task.; **task**: nhiệm vụ cụ thể mà prompt yêu cầu model hoàn thành.
...
> **Đáp án:** B
> **Giải thích:** Max iterations là safeguard cơ bản cùng với timeout và xử lý lỗi.
> **Thuật ngữ:** **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.
...
> **Đáp án:** D
> **Giải thích:** Hybrid giúp giữ chi phí thấp ở đường đơn giản và chỉ kích hoạt agent khi cần.
> **Thuật ngữ:** **chatbot**: hệ thống hội thoại thường trả lời bằng văn bản theo context của lượt chat.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.
...
> **Đáp án:** C
> **Giải thích:** Agentic Fit xem xét multi-step reasoning, tool use và quyết định động trước khi nâng cấp kiến trúc.
> **Thuật ngữ:** **reasoning**: suy luận; phân tích mục tiêu và chọn bước tiếp theo.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.
...
> **Đáp án:** B
> **Giải thích:** Nếu search_flights và get_weather không phụ thuộc dữ liệu nhau, parallel calling tiết kiệm thời gian.
> **Thuật ngữ:** **latency**: độ trễ; thời gian từ lúc gửi yêu cầu đến khi nhận kết quả.
...
> **Đáp án:** C
> **Giải thích:** Final answer không khớp observation là dấu hiệu hallucination hoặc lỗi tổng hợp.
> **Thuật ngữ:** **observation**: quan sát; kết quả tool hoặc môi trường trả lại cho agent.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.
...
> **Đáp án:** D
> **Giải thích:** Agent nên không bịa dữ liệu; có thể retry có giới hạn, fallback hoặc nói rõ chưa đủ evidence.
> **Thuật ngữ:** **graceful degradation**: suy giảm có kiểm soát; khi thiếu dữ liệu/lỗi, hệ thống nói rõ và dùng fallback an toàn.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.
...
> **Đáp án:** C
> **Giải thích:** Agentic Fit thấp cho thấy độ phức tạp của agent chưa tạo thêm giá trị.
> **Thuật ngữ:** **reasoning**: suy luận; phân tích mục tiêu và chọn bước tiếp theo.; **baseline**: mốc chuẩn dùng để so sánh với cách làm hiện tại.; **chatbot**: hệ thống hội thoại thường trả lời bằng văn bản theo context của lượt chat.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.
...
> **Đáp án:** D
> **Giải thích:** Timeout giới hạn thời gian chờ; retry và fallback cần được thiết kế bổ sung.
> **Thuật ngữ:** **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.
...
> **Đáp án:** A
> **Giải thích:** Không nên dùng agent cho bài toán một bước, không tool và yêu cầu deterministic cao.
> **Thuật ngữ:** **chatbot**: hệ thống hội thoại thường trả lời bằng văn bản theo context của lượt chat.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.; **LLM**: Large Language Model — mô hình ngôn ngữ lớn.
...
> **Đáp án:** B
> **Giải thích:** Tool name là tín hiệu phân loại đầu tiên; tên rõ giúp model định hướng chức năng.
> **Thuật ngữ:** Không có thuật ngữ tiếng Anh chuyên môn mới cần giải thích trong câu này.
...
> **Đáp án:** D
> **Giải thích:** Action gửi yêu cầu tới tool; Observation là dữ liệu môi trường trả lại cho agent.
> **Thuật ngữ:** **observation**: quan sát; kết quả tool hoặc môi trường trả lại cho agent.; **action**: hành động được agent chọn để tiến tới mục tiêu.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.; **ReAct**: Reasoning + Acting — suy luận kết hợp hành động theo vòng Thought → Action → Observation.
...
> **Đáp án:** C
> **Giải thích:** Mỗi bước reasoning hoặc synthesis có thể phát sinh thêm token và latency.
> **Thuật ngữ:** **reasoning**: suy luận; phân tích mục tiêu và chọn bước tiếp theo.; **latency**: độ trễ; thời gian từ lúc gửi yêu cầu đến khi nhận kết quả.; **chatbot**: hệ thống hội thoại thường trả lời bằng văn bản theo context của lượt chat.; **token**: đơn vị văn bản mà LLM trực tiếp xử lý và thường dùng để tính chi phí.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.; **LLM**: Large Language Model — mô hình ngôn ngữ lớn.
...
> **Đáp án:** B
> **Giải thích:** Day 3 chỉ rõ thứ tự tool, mã IATA và sự lệch evidence là ba loại lỗi cần sửa khác nhau.
> **Thuật ngữ:** Không có thuật ngữ tiếng Anh chuyên môn mới cần giải thích trong câu này.
...
> **Đáp án:** C
> **Giải thích:** Orchestration nên dựa trên dependency graph: parallel ở nhánh độc lập, sequential ở nhánh phụ thuộc.
> **Thuật ngữ:** Không có thuật ngữ tiếng Anh chuyên môn mới cần giải thích trong câu này.
...
> **Đáp án:** D
> **Giải thích:** Nhiều safeguard cần đi cùng nhau vì một vòng lặp không kiểm soát có thể đốt token và side effect.
...
> **Đáp án:** A
> **Giải thích:** Retry cần phân biệt transient/permanent failure và giới hạn số lần để bảo vệ cost.
> **Thuật ngữ:** Không có thuật ngữ tiếng Anh chuyên môn mới cần giải thích trong câu này.
...
> **Đáp án:** B
> **Giải thích:** Goal và evidence cần khớp; knowledge tĩnh không thay thế được dữ liệu realtime.
...
> **Đáp án:** C
> **Giải thích:** Hybrid thường dùng reasoning/control của ReAct cùng format tool call có cấu trúc.
> **Thuật ngữ:** **reasoning**: suy luận; phân tích mục tiêu và chọn bước tiếp theo.; **ReAct**: Reasoning + Acting — suy luận kết hợp hành động theo vòng Thought → Action → Observation.
...
> **Đáp án:** D
> **Giải thích:** Agent có nhiều call hơn; hybrid route giúp tránh dùng độ phức tạp không cần thiết.
> **Thuật ngữ:** **latency**: độ trễ; thời gian từ lúc gửi yêu cầu đến khi nhận kết quả.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.
...
> **Đáp án:** A
> **Giải thích:** Task dài làm tăng rủi ro mất state, loop, retry trùng và cần durable execution.
> **Thuật ngữ:** **side effect**: tác động bên ngoài như gửi email, xóa dữ liệu, trừ tiền hoặc đặt vé.; **task**: nhiệm vụ cụ thể mà prompt yêu cầu model hoàn thành.
...
> **Đáp án:** B
> **Giải thích:** Day 3 nêu reasoning quality, tool selection, arguments, error handling và stopping là các điểm cần xem.
> **Thuật ngữ:** **evaluation**: đánh giá chất lượng hệ thống trên bộ case và tiêu chí đã định.; **reasoning**: suy luận; phân tích mục tiêu và chọn bước tiếp theo.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.; **eval**: viết tắt của evaluation — đánh giá chất lượng AI.
...
> **Đáp án:** D
> **Giải thích:** Agent cost scale theo số call và token mỗi call; cần napkin math trước khi mở rộng.
> **Thuật ngữ:** **latency**: độ trễ; thời gian từ lúc gửi yêu cầu đến khi nhận kết quả.; **chatbot**: hệ thống hội thoại thường trả lời bằng văn bản theo context của lượt chat.; **token**: đơn vị văn bản mà LLM trực tiếp xử lý và thường dùng để tính chi phí.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.; **LLM**: Large Language Model — mô hình ngôn ngữ lớn.
...
> **Đáp án:** B
> **Giải thích:** Knowledge data là tri thức nền versioned hoặc ít thay đổi mà agent cần tra cứu.
> **Thuật ngữ:** **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.
...
> **Đáp án:** C
> **Giải thích:** Operational data như order status hoặc tồn kho cần freshness cao hơn knowledge data.
> **Thuật ngữ:** **freshness**: độ mới của dữ liệu so với thời điểm user cần sử dụng.
...
> **Đáp án:** D
> **Giải thích:** Contextual data thường ngắn và được inject trực tiếp cho task hiện tại.
> **Thuật ngữ:** **task**: nhiệm vụ cụ thể mà prompt yêu cầu model hoàn thành.
...
> **Đáp án:** A
> **Giải thích:** Chất lượng data giới hạn chất lượng retrieval và câu trả lời, kể cả khi model mạnh.
> **Thuật ngữ:** **retrieval**: truy xuất; tìm các chunk liên quan từ kho dữ liệu.; **AI**: Artificial Intelligence — trí tuệ nhân tạo.
...
> **Đáp án:** C
> **Giải thích:** Embedding tạo biểu diễn số để so sánh các đoạn có ý nghĩa gần nhau.
> **Thuật ngữ:** **embedding**: vector số biểu diễn ngữ nghĩa của văn bản, hình ảnh hoặc dữ liệu khác.
...
> **Đáp án:** D
> **Giải thích:** Cosine similarity so sánh góc giữa vector query và vector tài liệu.
> **Thuật ngữ:** **cosine similarity**: độ tương đồng cosine; đo độ gần về hướng giữa hai vector.
...
> **Đáp án:** A
> **Giải thích:** Kho vector cần giữ nội dung và metadata như source, category, date để trả lời có ngữ cảnh.
> **Thuật ngữ:** **vector store**: kho lưu vector, nội dung gốc và metadata để tìm kiếm theo ngữ nghĩa.; **metadata**: siêu dữ liệu như nguồn, loại, ngày, khu vực hoặc quyền của một chunk.
...
> **Đáp án:** B
> **Giải thích:** Top-k quyết định bao nhiêu chunk đầu tiên được đưa vào các bước tiếp theo.
> **Thuật ngữ:** **retrieval**: truy xuất; tìm các chunk liên quan từ kho dữ liệu.; **top-k**: số lượng kết quả gần nhất được lấy về trong một lần retrieval.
...
> **Đáp án:** D
> **Giải thích:** Filter theo category, region, date hoặc ACL giúp tránh lấy nhầm tài liệu.
> **Thuật ngữ:** **retrieval**: truy xuất; tìm các chunk liên quan từ kho dữ liệu.; **metadata**: siêu dữ liệu như nguồn, loại, ngày, khu vực hoặc quyền của một chunk.; **ACL**: Access Control List — danh sách quyền truy cập gắn với tài nguyên hoặc dữ liệu.
...
> **Đáp án:** C
> **Giải thích:** Inventory giúp biết dữ liệu nào phù hợp retrieval và trách nhiệm quản trị thuộc về ai.
> **Thuật ngữ:** **retrieval**: truy xuất; tìm các chunk liên quan từ kho dữ liệu.; **freshness**: độ mới của dữ liệu so với thời điểm user cần sử dụng.; **PII**: Personally Identifiable Information — dữ liệu có thể nhận diện một cá nhân.; **AI**: Artificial Intelligence — trí tuệ nhân tạo.
...
> **Đáp án:** B
> **Giải thích:** Tách knowledge và operational data giúp chọn cơ chế cập nhật và truy vấn phù hợp.
> **Thuật ngữ:** **retrieval**: truy xuất; tìm các chunk liên quan từ kho dữ liệu.; **freshness**: độ mới của dữ liệu so với thời điểm user cần sử dụng.; **API**: Application Programming Interface — giao diện để phần mềm gửi yêu cầu và nhận kết quả từ dịch vụ.
...
> **Đáp án:** C
> **Giải thích:** Tên, số điện thoại hoặc định danh cá nhân có thể lan sang kho và trace nếu không xử lý sớm.
> **Thuật ngữ:** **vector store**: kho lưu vector, nội dung gốc và metadata để tìm kiếm theo ngữ nghĩa.; **embedding**: vector số biểu diễn ngữ nghĩa của văn bản, hình ảnh hoặc dữ liệu khác.; **PII**: Personally Identifiable Information — dữ liệu có thể nhận diện một cá nhân.
...
> **Đáp án:** D
> **Giải thích:** Không phải mọi sự kiện đều đáng nhớ; lifecycle cần chọn, lưu và truy xuất có chủ đích.
> **Thuật ngữ:** Không có thuật ngữ tiếng Anh chuyên môn mới cần giải thích trong câu này.
...
> **Đáp án:** A
> **Giải thích:** Attribute filter theo region hoặc quyền giúp tránh retrieval từ domain không được phép.
> **Thuật ngữ:** **retrieval**: truy xuất; tìm các chunk liên quan từ kho dữ liệu.; **metadata**: siêu dữ liệu như nguồn, loại, ngày, khu vực hoặc quyền của một chunk.; **ACL**: Access Control List — danh sách quyền truy cập gắn với tài nguyên hoặc dữ liệu.
...
> **Đáp án:** B
> **Giải thích:** Lấy thêm chunk không đồng nghĩa thêm bằng chứng hữu ích; cần threshold hoặc rerank.
> **Thuật ngữ:** **threshold**: ngưỡng điểm dùng để loại kết quả có mức liên quan quá thấp.; **context**: bối cảnh hoặc dữ liệu nền được đưa cho model để xử lý task.; **token**: đơn vị văn bản mà LLM trực tiếp xử lý và thường dùng để tính chi phí.; **top-k**: số lượng kết quả gần nhất được lấy về trong một lần retrieval.
...
> **Đáp án:** D
> **Giải thích:** Chunk nhỏ tìm trúng hơn nhưng có thể thiếu phần đầu/cuối của điều khoản.
> **Thuật ngữ:** Không có thuật ngữ tiếng Anh chuyên môn mới cần giải thích trong câu này.
...
> **Đáp án:** C
> **Giải thích:** Exact identifiers là điểm mạnh của sparse retrieval; hybrid cân bằng thêm ý nghĩa.
> **Thuật ngữ:** **sparse retrieval**: truy xuất thưa dựa trên khớp từ khóa, thường dùng inverted index.; **hybrid search**: tìm kiếm lai kết hợp dense semantic search với sparse keyword search.; **retrieval**: truy xuất; tìm các chunk liên quan từ kho dữ liệu.
...
> **Đáp án:** D
> **Giải thích:** Approximate Nearest Neighbor đánh đổi ít chính xác để giảm chi phí tìm kiếm ở quy mô lớn.
> **Thuật ngữ:** Không có thuật ngữ tiếng Anh chuyên môn mới cần giải thích trong câu này.
...
> **Đáp án:** A
> **Giải thích:** Managed service giảm công vận hành; self-managed phù hợp khi cần kiểm soát sâu hơn.
> **Thuật ngữ:** **retrieval**: truy xuất; tìm các chunk liên quan từ kho dữ liệu.
...
> **Đáp án:** B
> **Giải thích:** RAG nên dùng khi tri thức riêng, cập nhật hoặc cần audit; không phải câu hỏi nào cũng cần.
> **Thuật ngữ:** **retrieval**: truy xuất; tìm các chunk liên quan từ kho dữ liệu.; **latency**: độ trễ; thời gian từ lúc gửi yêu cầu đến khi nhận kết quả.; **RAG**: Retrieval-Augmented Generation — tạo sinh tăng cường bằng truy xuất tài liệu.
...
> **Đáp án:** B
> **Giải thích:** Table parsing cần bảo toàn quan hệ header-row; nếu không retrieval mất ngữ cảnh và trả sai.
> **Thuật ngữ:** **retrieval**: truy xuất; tìm các chunk liên quan từ kho dữ liệu.
...
> **Đáp án:** C
> **Giải thích:** Child chunk tăng precision khi search; parent chunk bổ sung context khi inject.
> **Thuật ngữ:** **retrieval**: truy xuất; tìm các chunk liên quan từ kho dữ liệu.; **precision**: tỷ lệ kết quả được dự đoán dương tính mà thực sự đúng.; **indexing**: lập chỉ mục; parse, clean, chunk, embed và lưu tài liệu để tìm kiếm.; **context**: bối cảnh hoặc dữ liệu nền được đưa cho model để xử lý task.
...
> **Đáp án:** D
> **Giải thích:** Post-filter có thể loại phần lớn top-k sau khi search và làm thiếu kết quả phù hợp.
> **Thuật ngữ:** **top-k**: số lượng kết quả gần nhất được lấy về trong một lần retrieval.
...
> **Đáp án:** A
> **Giải thích:** Hai trường phái bù điểm mù cho nhau; domain enterprise thường cần cả ý nghĩa và exact match.
> **Thuật ngữ:** **hybrid search**: tìm kiếm lai kết hợp dense semantic search với sparse keyword search.
...
> **Đáp án:** B
> **Giải thích:** Day 7 phân biệt file tạm với long-term memory có cơ chế store/retrieve rõ.
> **Thuật ngữ:** **long-term memory**: bộ nhớ dài hạn; thông tin được lưu để truy xuất lại qua nhiều phiên.; **prompt**: chỉ dẫn hoặc đầu vào bằng ngôn ngữ tự nhiên gửi cho model.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.
...
> **Đáp án:** C
> **Giải thích:** Metadata chỉ có giá trị khi pipeline thực sự áp dụng filter và kiểm tra quyền.
> **Thuật ngữ:** **metadata**: siêu dữ liệu như nguồn, loại, ngày, khu vực hoặc quyền của một chunk.; **context**: bối cảnh hoặc dữ liệu nền được đưa cho model để xử lý task.; **ACL**: Access Control List — danh sách quyền truy cập gắn với tài nguyên hoặc dữ liệu.
...
> **Đáp án:** D
> **Giải thích:** Data quality pyramid đi từ raw đến cleaned/structured; nạp rác làm retrieval sai dây chuyền.
> **Thuật ngữ:** **retrieval**: truy xuất; tìm các chunk liên quan từ kho dữ liệu.
...
> **Đáp án:** A
> **Giải thích:** ANN không phải exact brute force; nó phù hợp khi quy mô khiến tìm tuyến tính quá chậm.
> **Thuật ngữ:** Không có thuật ngữ tiếng Anh chuyên môn mới cần giải thích trong câu này.
...
> **Đáp án:** B
> **Giải thích:** Ba loại data cần đường truy xuất khác nhau; không nên ép operational data vào knowledge index tĩnh.
> **Thuật ngữ:** **retrieval**: truy xuất; tìm các chunk liên quan từ kho dữ liệu.; **API**: Application Programming Interface — giao diện để phần mềm gửi yêu cầu và nhận kết quả từ dịch vụ.; **RAG**: Retrieval-Augmented Generation — tạo sinh tăng cường bằng truy xuất tài liệu.
...
> **Đáp án:** C
> **Giải thích:** Day 7 nhấn mạnh data quality thường quan trọng hơn đổi model đắt hơn.
> **Thuật ngữ:** **retrieval**: truy xuất; tìm các chunk liên quan từ kho dữ liệu.; **context**: bối cảnh hoặc dữ liệu nền được đưa cho model để xử lý task.; **AI**: Artificial Intelligence — trí tuệ nhân tạo.
...
> **Đáp án:** B
> **Giải thích:** RAG kết hợp truy xuất tài liệu với khả năng sinh ngôn ngữ của LLM.
> **Thuật ngữ:** **generation**: tạo sinh; bước LLM tổng hợp câu trả lời từ context đã truy xuất.; **retrieval**: truy xuất; tìm các chunk liên quan từ kho dữ liệu.; **LLM**: Large Language Model — mô hình ngôn ngữ lớn.; **RAG**: Retrieval-Augmented Generation — tạo sinh tăng cường bằng truy xuất tài liệu.
...
> **Đáp án:** C
> **Giải thích:** Indexing chuẩn bị kho, retrieval tìm context, generation tổng hợp câu trả lời.
> **Thuật ngữ:** **generation**: tạo sinh; bước LLM tổng hợp câu trả lời từ context đã truy xuất.; **retrieval**: truy xuất; tìm các chunk liên quan từ kho dữ liệu.; **indexing**: lập chỉ mục; parse, clean, chunk, embed và lưu tài liệu để tìm kiếm.; **context**: bối cảnh hoặc dữ liệu nền được đưa cho model để xử lý task.; **RAG**: Retrieval-Augmented Generation — tạo sinh tăng cường bằng truy xuất tài liệu.
...
> **Đáp án:** D
> **Giải thích:** Slide so sánh fine-tuning với RAG: fine-tuning thiên về cách model nói, RAG thiên về tri thức cập nhật.
> **Thuật ngữ:** **fine-tuning**: tinh chỉnh model bằng dữ liệu chuyên biệt để thay đổi hành vi hoặc phong cách.; **RAG**: Retrieval-Augmented Generation — tạo sinh tăng cường bằng truy xuất tài liệu.
...
> **Đáp án:** A
> **Giải thích:** Indexing biến dữ liệu phi cấu trúc thành các chunk có vector và thông tin truy vết.
> **Thuật ngữ:** **metadata**: siêu dữ liệu như nguồn, loại, ngày, khu vực hoặc quyền của một chunk.; **indexing**: lập chỉ mục; parse, clean, chunk, embed và lưu tài liệu để tìm kiếm.
...
> **Đáp án:** C
> **Giải thích:** Query được embed và dùng để tìm các chunk gần nhất trong kho.
> **Thuật ngữ:** **retrieval**: truy xuất; tìm các chunk liên quan từ kho dữ liệu.; **context**: bối cảnh hoặc dữ liệu nền được đưa cho model để xử lý task.
...
> **Đáp án:** D
> **Giải thích:** Strict grounding giảm hallucination bằng cách buộc câu trả lời bám evidence.
> **Thuật ngữ:** **grounding**: neo câu trả lời vào evidence được cung cấp, không tự suy diễn ngoài nguồn.; **context**: bối cảnh hoặc dữ liệu nền được đưa cho model để xử lý task.; **RAG**: Retrieval-Augmented Generation — tạo sinh tăng cường bằng truy xuất tài liệu.
...
> **Đáp án:** A
> **Giải thích:** Chunk quá lớn gây nhiễu; chunk quá nhỏ có thể mất ngữ cảnh.
> **Thuật ngữ:** **retrieval**: truy xuất; tìm các chunk liên quan từ kho dữ liệu.; **context**: bối cảnh hoặc dữ liệu nền được đưa cho model để xử lý task.
...
> **Đáp án:** B
> **Giải thích:** Phần chồng lấn giữ lại một ít ngữ cảnh ở hai phía của ranh giới.
> **Thuật ngữ:** Không có thuật ngữ tiếng Anh chuyên môn mới cần giải thích trong câu này.
...
> **Đáp án:** D
> **Giải thích:** BM25 là thuật toán lexical/sparse mạnh khi cần khớp chính xác ID, mã lỗi hoặc tên.
> **Thuật ngữ:** **sparse retrieval**: truy xuất thưa dựa trên khớp từ khóa, thường dùng inverted index.; **retrieval**: truy xuất; tìm các chunk liên quan từ kho dữ liệu.; **BM25**: thuật toán sparse search chấm mức phù hợp của từ khóa trong tài liệu.
...
> **Đáp án:** C
> **Giải thích:** Citation tăng auditability và giúp phát hiện câu trả lời không có evidence.
> **Thuật ngữ:** **citation**: trích dẫn nguồn; chỉ ra tài liệu hoặc dòng bằng chứng dùng cho câu trả lời.; **RAG**: Retrieval-Augmented Generation — tạo sinh tăng cường bằng truy xuất tài liệu.
...
> **Đáp án:** B
> **Giải thích:** RAG cho phép cập nhật nguồn riêng mà không phải huấn luyện lại toàn bộ model.
> **Thuật ngữ:** **fine-tuning**: tinh chỉnh model bằng dữ liệu chuyên biệt để thay đổi hành vi hoặc phong cách.; **freshness**: độ mới của dữ liệu so với thời điểm user cần sử dụng.; **RAG**: Retrieval-Augmented Generation — tạo sinh tăng cường bằng truy xuất tài liệu.
...
> **Đáp án:** C
> **Giải thích:** Sinh biến thể query giúp không bỏ sót tài liệu vì khác cách diễn đạt.
> **Thuật ngữ:** **query expansion**: mở rộng query bằng từ đồng nghĩa, thuật ngữ hoặc biến thể để tăng recall.; **recall**: tỷ lệ các trường hợp đúng mà hệ thống tìm được.
...
> **Đáp án:** D
> **Giải thích:** Chia câu hỏi thành sub-query cho phép retrieval từng phần rồi tổng hợp.
> **Thuật ngữ:** **query decomposition**: chia câu hỏi phức tạp thành các sub-query nhỏ hơn.; **retrieval**: truy xuất; tìm các chunk liên quan từ kho dữ liệu.
...
> **Đáp án:** A
> **Giải thích:** Hypothetical answer có hình thức gần document hơn query ngắn, hỗ trợ tìm kiếm; không phải evidence cuối.
> **Thuật ngữ:** **HyDE**: Hypothetical Document Embeddings — tạo văn bản giả định để đưa query gần không gian tài liệu.
...
> **Đáp án:** B
> **Giải thích:** Dense bắt ý nghĩa; sparse bắt keyword, ID và thuật ngữ exact.
> **Thuật ngữ:** **hybrid search**: tìm kiếm lai kết hợp dense semantic search với sparse keyword search.
...
> **Đáp án:** C
> **Giải thích:** Reciprocal Rank Fusion dùng rank để tránh phải so trực tiếp score khác thang đo.
> **Thuật ngữ:** **RRF**: Reciprocal Rank Fusion — gộp kết quả dựa trên thứ hạng từ nhiều retriever.
...
> **Đáp án:** D
> **Giải thích:** Slide gợi ý domain code/log/pháp lý ưu tiên keyword chính xác hơn semantic mơ hồ.
> **Thuật ngữ:** **BM25**: thuật toán sparse search chấm mức phù hợp của từ khóa trong tài liệu.
...
> **Đáp án:** A
> **Giải thích:** Bi-encoder phù hợp retrieve nhanh; cross-encoder rerank ít candidate với độ chính xác cao hơn.
> **Thuật ngữ:** **cross-encoder**: model đọc query và document cùng nhau để chấm relevance chính xác hơn.; **bi-encoder**: model encode query và document riêng để tìm kiếm nhanh bằng vector.
...
> **Đáp án:** B
> **Giải thích:** Maximum Marginal Relevance cân bằng relevance với diversity để không đưa nhiều bản sao.
> **Thuật ngữ:** **context**: bối cảnh hoặc dữ liệu nền được đưa cho model để xử lý task.; **top-k**: số lượng kết quả gần nhất được lấy về trong một lần retrieval.; **MMR**: Maximum Marginal Relevance — cân bằng relevance với độ đa dạng, giảm chunk trùng lặp.
...
> **Đáp án:** D
> **Giải thích:** Nếu evidence đã vào formatted_context mà câu trả lời sai, lỗi nằm sau retrieval.
> **Thuật ngữ:** **temperature**: tham số sampling điều khiển mức đa dạng/ngẫu nhiên của output.; **generation**: tạo sinh; bước LLM tổng hợp câu trả lời từ context đã truy xuất.; **retrieval**: truy xuất; tìm các chunk liên quan từ kho dữ liệu.; **grounding**: neo câu trả lời vào evidence được cung cấp, không tự suy diễn ngoài nguồn.; **context**: bối cảnh hoặc dữ liệu nền được đưa cho model để xử lý task.; **prompt**: chỉ dẫn hoặc đầu vào bằng ngôn ngữ tự nhiên gửi cho model.
...
> **Đáp án:** B
> **Giải thích:** Nếu nội dung đã bị đọc sai từ nguồn thì mọi embedding và generation phía sau đều bị ảnh hưởng.
> **Thuật ngữ:** **generation**: tạo sinh; bước LLM tổng hợp câu trả lời từ context đã truy xuất.; **embedding**: vector số biểu diễn ngữ nghĩa của văn bản, hình ảnh hoặc dữ liệu khác.
...
> **Đáp án:** C
> **Giải thích:** Cần mở rộng thuật ngữ, chia sub-query và lọc domain/region để tăng recall có kiểm soát.
> **Thuật ngữ:** **query expansion**: mở rộng query bằng từ đồng nghĩa, thuật ngữ hoặc biến thể để tăng recall.; **metadata**: siêu dữ liệu như nguồn, loại, ngày, khu vực hoặc quyền của một chunk.; **recall**: tỷ lệ các trường hợp đúng mà hệ thống tìm được.
...
> **Đáp án:** D
> **Giải thích:** HyDE là query transformation; evidence thật vẫn phải đến từ tài liệu retrieved.
> **Thuật ngữ:** **HyDE**: Hypothetical Document Embeddings — tạo văn bản giả định để đưa query gần không gian tài liệu.
...
> **Đáp án:** A
> **Giải thích:** RRF dùng vị trí xếp hạng và tránh bài toán 'cam và táo' giữa hai score.
> **Thuật ngữ:** **BM25**: thuật toán sparse search chấm mức phù hợp của từ khóa trong tài liệu.; **RRF**: Reciprocal Rank Fusion — gộp kết quả dựa trên thứ hạng từ nhiều retriever.
...
> **Đáp án:** B
> **Giải thích:** Cross-encoder tối ưu relevance; MMR bổ sung mục tiêu diversity khi các kết quả trùng.
> **Thuật ngữ:** **cross-encoder**: model đọc query và document cùng nhau để chấm relevance chính xác hơn.; **context**: bối cảnh hoặc dữ liệu nền được đưa cho model để xử lý task.; **MMR**: Maximum Marginal Relevance — cân bằng relevance với độ đa dạng, giảm chunk trùng lặp.
...
> **Đáp án:** C
> **Giải thích:** Giảm nhiễu và đặt evidence quan trọng ở vị trí được chú ý hơn cải thiện generation.
> **Thuật ngữ:** **generation**: tạo sinh; bước LLM tổng hợp câu trả lời từ context đã truy xuất.; **context**: bối cảnh hoặc dữ liệu nền được đưa cho model để xử lý task.; **prompt**: chỉ dẫn hoặc đầu vào bằng ngôn ngữ tự nhiên gửi cho model.
...
> **Đáp án:** D
> **Giải thích:** Conflict resolution cần version/effective date; nếu không giải quyết được, agent nên nói rõ giới hạn.
> **Thuật ngữ:** **generation**: tạo sinh; bước LLM tổng hợp câu trả lời từ context đã truy xuất.; **metadata**: siêu dữ liệu như nguồn, loại, ngày, khu vực hoặc quyền của một chunk.; **context**: bối cảnh hoặc dữ liệu nền được đưa cho model để xử lý task.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.
...
> **Đáp án:** B
> **Giải thích:** Nếu evidence không vào context, generation không thể grounded; cần sửa retrieval trước.
> **Thuật ngữ:** **generation**: tạo sinh; bước LLM tổng hợp câu trả lời từ context đã truy xuất.; **retrieval**: truy xuất; tìm các chunk liên quan từ kho dữ liệu.; **indexing**: lập chỉ mục; parse, clean, chunk, embed và lưu tài liệu để tìm kiếm.; **context**: bối cảnh hoặc dữ liệu nền được đưa cho model để xử lý task.
...
> **Đáp án:** B
> **Giải thích:** Context bottleneck và quá nhiều trách nhiệm là dấu hiệu cân nhắc multi-agent.
> **Thuật ngữ:** **multi-agent**: hệ nhiều agent được phân vai và phối hợp cho cùng một nhiệm vụ.; **context**: bối cảnh hoặc dữ liệu nền được đưa cho model để xử lý task.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.
...
> **Đáp án:** C
> **Giải thích:** Multi-agent không mặc định tốt hơn; giá trị là phân vai hợp lý cho bài toán phức tạp.
> **Thuật ngữ:** **multi-agent**: hệ nhiều agent được phân vai và phối hợp cho cùng một nhiệm vụ.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.
...
> **Đáp án:** D
> **Giải thích:** Supervisor chịu trách nhiệm điều phối, còn worker xử lý năng lực hẹp hơn.
> **Thuật ngữ:** **supervisor**: agent điều phối; route task tới worker và tổng hợp kết quả.; **worker**: agent chuyên trách một năng lực hẹp như retrieve, gọi tool hoặc summarize.
...
> **Đáp án:** A
> **Giải thích:** Worker hẹp giúp route rõ, dễ test và dễ xác định lỗi.
> **Thuật ngữ:** **multi-agent**: hệ nhiều agent được phân vai và phối hợp cho cùng một nhiệm vụ.; **worker**: agent chuyên trách một năng lực hẹp như retrieve, gọi tool hoặc summarize.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.
...
> **Đáp án:** C
> **Giải thích:** MCP giảm việc viết adapter riêng cho từng tool và tạo giao thức client/server thống nhất.
> **Thuật ngữ:** **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.; **MCP**: Model Context Protocol — chuẩn kết nối agent với tools và resources bên ngoài.
...
> **Đáp án:** D
> **Giải thích:** Tools phục vụ hành động; resources cung cấp dữ liệu hoặc tài nguyên để đọc.
> **Thuật ngữ:** **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.; **MCP**: Model Context Protocol — chuẩn kết nối agent với tools và resources bên ngoài.
...
> **Đáp án:** A
> **Giải thích:** Slide nhấn mạnh MCP là agent–tool, còn A2A là agent–agent communication.
> **Thuật ngữ:** **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.; **MCP**: Model Context Protocol — chuẩn kết nối agent với tools và resources bên ngoài.; **A2A**: Agent-to-Agent — giao tiếp giữa các agent qua message contract.
...
> **Đáp án:** B
> **Giải thích:** Contract rõ giảm context thiếu, output lệch và số lần supervisor phải gọi lại.
> **Thuật ngữ:** **supervisor**: agent điều phối; route task tới worker và tổng hợp kết quả.; **context**: bối cảnh hoặc dữ liệu nền được đưa cho model để xử lý task.; **worker**: agent chuyên trách một năng lực hẹp như retrieve, gọi tool hoặc summarize.; **A2A**: Agent-to-Agent — giao tiếp giữa các agent qua message contract.
...
> **Đáp án:** D
> **Giải thích:** State là vùng thông tin chung giúp nodes phối hợp trong một workflow.
> **Thuật ngữ:** **shared state**: trạng thái dùng chung giữa các node/agent trong một workflow.; **workflow**: quy trình gồm các bước và bàn giao để hoàn thành một công việc.; **task**: nhiệm vụ cụ thể mà prompt yêu cầu model hoàn thành.
...
> **Đáp án:** C
> **Giải thích:** LangGraph mô hình hóa logic bằng nodes, edges và state.
> **Thuật ngữ:** **LangGraph**: framework biểu diễn workflow agent bằng state, node, edge và conditional routing.
...
> **Đáp án:** B
> **Giải thích:** Phân vai rõ giúp route, debug và kiểm tra từng giai đoạn.
> **Thuật ngữ:** **supervisor**: agent điều phối; route task tới worker và tổng hợp kết quả.; **worker**: agent chuyên trách một năng lực hẹp như retrieve, gọi tool hoặc summarize.; **task**: nhiệm vụ cụ thể mà prompt yêu cầu model hoàn thành.
...
> **Đáp án:** C
> **Giải thích:** Task độc lập phù hợp parallel execution, nhưng vẫn cần schema và timeout rõ.
> **Thuật ngữ:** **supervisor**: agent điều phối; route task tới worker và tổng hợp kết quả.; **latency**: độ trễ; thời gian từ lúc gửi yêu cầu đến khi nhận kết quả.; **worker**: agent chuyên trách một năng lực hẹp như retrieve, gọi tool hoặc summarize.; **task**: nhiệm vụ cụ thể mà prompt yêu cầu model hoàn thành.
...
> **Đáp án:** D
> **Giải thích:** Shared state thuận tiện cho graph orchestration; message passing có thể cách ly tốt hơn giữa các thành phần.
> **Thuật ngữ:** **shared state**: trạng thái dùng chung giữa các node/agent trong một workflow.
...
> **Đáp án:** A
> **Giải thích:** Luồng discovery giúp client biết danh sách, mô tả và schema tool mà server công bố.
> **Thuật ngữ:** **MCP**: Model Context Protocol — chuẩn kết nối agent với tools và resources bên ngoài.
...
> **Đáp án:** B
> **Giải thích:** Contract cần nêu nhiệm vụ, context, format và bằng chứng để worker trả kết quả dùng được.
> **Thuật ngữ:** **supervisor**: agent điều phối; route task tới worker và tổng hợp kết quả.; **context**: bối cảnh hoặc dữ liệu nền được đưa cho model để xử lý task.; **worker**: agent chuyên trách một năng lực hẹp như retrieve, gọi tool hoặc summarize.; **A2A**: Agent-to-Agent — giao tiếp giữa các agent qua message contract.
...
> **Đáp án:** C
> **Giải thích:** Sync đơn giản để hiểu và debug khi luồng cần kết quả trước khi tiếp tục.
> **Thuật ngữ:** **supervisor**: agent điều phối; route task tới worker và tổng hợp kết quả.; **A2A**: Agent-to-Agent — giao tiếp giữa các agent qua message contract.
...
> **Đáp án:** D
> **Giải thích:** Quyền và dữ liệu cần giới hạn; worker chạm external tool phải có validation.
> **Thuật ngữ:** **worker**: agent chuyên trách một năng lực hẹp như retrieve, gọi tool hoặc summarize.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.; **AI**: Artificial Intelligence — trí tuệ nhân tạo.
...
> **Đáp án:** A
> **Giải thích:** Router/conditional edge cho phép graph rẽ nhánh theo tool call, trạng thái hoặc điều kiện kết thúc.
> **Thuật ngữ:** **LangGraph**: framework biểu diễn workflow agent bằng state, node, edge và conditional routing.
...
> **Đáp án:** B
> **Giải thích:** Trace cần cho biết ai nhận gì, được gọi vì sao và trả kết quả nào.
> **Thuật ngữ:** **multi-agent**: hệ nhiều agent được phân vai và phối hợp cho cùng một nhiệm vụ.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.; **AI**: Artificial Intelligence — trí tuệ nhân tạo.
...
> **Đáp án:** D
> **Giải thích:** Multi-agent là trade-off; số agent không phải thước đo chất lượng.
> **Thuật ngữ:** **multi-agent**: hệ nhiều agent được phân vai và phối hợp cho cùng một nhiệm vụ.; **latency**: độ trễ; thời gian từ lúc gửi yêu cầu đến khi nhận kết quả.; **worker**: agent chuyên trách một năng lực hẹp như retrieve, gọi tool hoặc summarize.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.
...
> **Đáp án:** B
> **Giải thích:** Context bottleneck, nhiều vai trò và nhu cầu truy nguyên lỗi là lý do thực dụng để phân chia.
> **Thuật ngữ:** **context**: bối cảnh hoặc dữ liệu nền được đưa cho model để xử lý task.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.; **API**: Application Programming Interface — giao diện để phần mềm gửi yêu cầu và nhận kết quả từ dịch vụ.
...
> **Đáp án:** C
> **Giải thích:** Pipeline dành cho flow tuyến tính; supervisor phù hợp khi route/subtask thay đổi theo input.
> **Thuật ngữ:** **multi-agent**: hệ nhiều agent được phân vai và phối hợp cho cùng một nhiệm vụ.; **supervisor**: agent điều phối; route task tới worker và tổng hợp kết quả.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.
...
> **Đáp án:** D
> **Giải thích:** Supervisor quá tải làm mất lợi ích phân vai và trở lại single-agent khó debug.
> **Thuật ngữ:** **supervisor**: agent điều phối; route task tới worker và tổng hợp kết quả.; **worker**: agent chuyên trách một năng lực hẹp như retrieve, gọi tool hoặc summarize.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.
...
> **Đáp án:** A
> **Giải thích:** Message passing và data minimization giảm blast radius so với chia toàn bộ state.
> **Thuật ngữ:** **shared state**: trạng thái dùng chung giữa các node/agent trong một workflow.; **worker**: agent chuyên trách một năng lực hẹp như retrieve, gọi tool hoặc summarize.; **PII**: Personally Identifiable Information — dữ liệu có thể nhận diện một cá nhân.
...
> **Đáp án:** B
> **Giải thích:** MCP client cần tools/list và schema để biết cách gọi; contract không rõ dẫn đến arguments sai.
> **Thuật ngữ:** **MCP**: Model Context Protocol — chuẩn kết nối agent với tools và resources bên ngoài.
...
> **Đáp án:** C
> **Giải thích:** Contract cần định nghĩa output có thể tổng hợp và audit, không chỉ một câu mô tả chung.
> **Thuật ngữ:** **supervisor**: agent điều phối; route task tới worker và tổng hợp kết quả.; **worker**: agent chuyên trách một năng lực hẹp như retrieve, gọi tool hoặc summarize.; **A2A**: Agent-to-Agent — giao tiếp giữa các agent qua message contract.
...
> **Đáp án:** D
> **Giải thích:** Async tách vòng đời task dài khỏi request và cho phép theo dõi trạng thái.
> **Thuật ngữ:** **supervisor**: agent điều phối; route task tới worker và tổng hợp kết quả.; **worker**: agent chuyên trách một năng lực hẹp như retrieve, gọi tool hoặc summarize.; **task**: nhiệm vụ cụ thể mà prompt yêu cầu model hoàn thành.
...
> **Đáp án:** B
> **Giải thích:** Cả hai đều có Extract, Transform và Load; điểm khác là vị trí transform.
> **Thuật ngữ:** **ETL**: Extract–Transform–Load — lấy, biến đổi rồi nạp dữ liệu.; **ELT**: Extract–Load–Transform — lấy, nạp raw trước rồi biến đổi trong kho đích.
...
> **Đáp án:** C
> **Giải thích:** Batch phù hợp khi có thể chấp nhận độ trễ và muốn vận hành theo lịch.
> **Thuật ngữ:** **streaming**: trả kết quả từng phần ngay khi sinh, thay vì chờ toàn bộ output.; **batch**: xử lý dữ liệu theo lô vào thời điểm định trước.
...
> **Đáp án:** D
> **Giải thích:** Streaming xử lý dòng event liên tục và giảm độ trễ so với chạy theo lô.
> **Thuật ngữ:** **streaming**: trả kết quả từng phần ngay khi sinh, thay vì chờ toàn bộ output.; **batch**: xử lý dữ liệu theo lô vào thời điểm định trước.
...
> **Đáp án:** A
> **Giải thích:** Change Data Capture ghi nhận thay đổi thay vì quét lại toàn bộ bảng mỗi lần.
> **Thuật ngữ:** **snapshot**: ảnh chụp dữ liệu tại một thời điểm, thường dùng để quét theo lịch.; **CDC**: Change Data Capture — ghi nhận các bản ghi được tạo, sửa hoặc xóa.
...
> **Đáp án:** C
> **Giải thích:** Backoff giảm áp lực lên dịch vụ đang giới hạn tốc độ; jitter giúp tránh retry đồng thời.
> **Thuật ngữ:** **exponential backoff**: tăng dần thời gian chờ giữa các lần retry, thường theo cấp số nhân.; **jitter**: độ ngẫu nhiên nhỏ cộng vào thời gian retry để tránh nhiều client retry cùng lúc.
...
> **Đáp án:** D
> **Giải thích:** Cursor tiếp tục từ dấu mốc ổn định hơn khi dữ liệu nguồn thay đổi trong lúc đọc.
> **Thuật ngữ:** **pagination**: chia kết quả lớn thành nhiều trang hoặc batch nhỏ để đọc tuần tự.
...
> **Đáp án:** A
> **Giải thích:** Dead-letter queue giữ message không xử lý được để không chặn toàn bộ dòng dữ liệu.
> **Thuật ngữ:** **dead-letter queue**: DLQ — hàng đợi chứa message xử lý thất bại để điều tra hoặc replay.
...
> **Đáp án:** B
> **Giải thích:** Data contract là thỏa thuận có thể kiểm tra giữa nguồn và bên nhận dữ liệu.
> **Thuật ngữ:** **data contract**: thỏa thuận về schema, kiểu dữ liệu, giá trị hợp lệ và trách nhiệm giữa các bên.
...
> **Đáp án:** D
> **Giải thích:** Drift là tín hiệu cần điều tra; nó không tự kết luận dữ liệu mới đúng hay sai.
> **Thuật ngữ:** **drift**: sự thay đổi phân phối dữ liệu hoặc hành vi so với mốc tham chiếu.
...
> **Đáp án:** C
> **Giải thích:** Upsert theo natural key như doc_id và version giúp lần chạy lại không nhân đôi dữ liệu.
> **Thuật ngữ:** **idempotency**: tính lặp an toàn; chạy lại cùng request không tạo thêm side effect hoặc bản ghi trùng.
...
> **Đáp án:** B
> **Giải thích:** ETL lọc và chuẩn hóa sớm; ELT giữ raw để transform sau, thường linh hoạt hơn cho warehouse.
> **Thuật ngữ:** **ETL**: Extract–Transform–Load — lấy, biến đổi rồi nạp dữ liệu.; **ELT**: Extract–Load–Transform — lấy, nạp raw trước rồi biến đổi trong kho đích.
...
> **Đáp án:** C
> **Giải thích:** Operational freshness cao ưu tiên event-driven ingestion hơn batch chậm.
> **Thuật ngữ:** **streaming**: trả kết quả từng phần ngay khi sinh, thay vì chờ toàn bộ output.; **freshness**: độ mới của dữ liệu so với thời điểm user cần sử dụng.; **batch**: xử lý dữ liệu theo lô vào thời điểm định trước.; **CDC**: Change Data Capture — ghi nhận các bản ghi được tạo, sửa hoặc xóa.
...
> **Đáp án:** D
> **Giải thích:** CDC tiết kiệm công đọc và truyền dữ liệu, nhưng phải xử lý đầy đủ create/update/delete.
> **Thuật ngữ:** **snapshot**: ảnh chụp dữ liệu tại một thời điểm, thường dùng để quét theo lịch.; **CDC**: Change Data Capture — ghi nhận các bản ghi được tạo, sửa hoặc xóa.
...
> **Đáp án:** A
> **Giải thích:** Retry-After là tín hiệu từ server; exponential backoff và jitter tránh tạo bão retry.
> **Thuật ngữ:** **exponential backoff**: tăng dần thời gian chờ giữa các lần retry, thường theo cấp số nhân.; **jitter**: độ ngẫu nhiên nhỏ cộng vào thời gian retry để tránh nhiều client retry cùng lúc.; **API**: Application Programming Interface — giao diện để phần mềm gửi yêu cầu và nhận kết quả từ dịch vụ.
...
> **Đáp án:** B
> **Giải thích:** Offset phụ thuộc vị trí; dữ liệu đổi có thể đẩy dòng sang trang trước hoặc sau.
> **Thuật ngữ:** **pagination**: chia kết quả lớn thành nhiều trang hoặc batch nhỏ để đọc tuần tự.
...
> **Đáp án:** C
> **Giải thích:** File có thể đổi bytes nhưng giữ version; hoặc version đổi mà bytes xử lý không như mong muốn.
> **Thuật ngữ:** Không có thuật ngữ tiếng Anh chuyên môn mới cần giải thích trong câu này.
...
> **Đáp án:** D
> **Giải thích:** Queue, giới hạn tốc độ hoặc pause producer bảo vệ consumer khi lưu lượng tăng.
> **Thuật ngữ:** **backpressure**: cơ chế làm chậm producer khi consumer hoặc downstream quá tải.
...
> **Đáp án:** A
> **Giải thích:** Schema có thể pass nhưng dữ liệu thiếu; cần theo dõi completeness và nguyên nhân upstream.
> **Thuật ngữ:** Không có thuật ngữ tiếng Anh chuyên môn mới cần giải thích trong câu này.
...
> **Đáp án:** B
> **Giải thích:** Job thành công nhưng không có dữ liệu mới có thể là freshness hoặc publish/cache swap lỗi.
> **Thuật ngữ:** **freshness**: độ mới của dữ liệu so với thời điểm user cần sử dụng.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.
...
> **Đáp án:** D
> **Giải thích:** Gate là điểm quyết định; dữ liệu nguy hiểm cần bị chặn và có dấu vết xử lý.
> **Thuật ngữ:** **quality gate**: điểm kiểm tra quyết định dữ liệu có được đi tiếp, quarantine hoặc dừng pipeline.
...
> **Đáp án:** B
> **Giải thích:** Một quyết định/lỗi đầu nguồn có thể làm hậu quả lớn dần qua các tầng.
> **Thuật ngữ:** **retrieval**: truy xuất; tìm các chunk liên quan từ kho dữ liệu.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.
...
> **Đáp án:** C
> **Giải thích:** ELT phù hợp khi warehouse có khả năng xử lý và cần giữ raw; quality gate downstream vẫn cần.
> **Thuật ngữ:** **quality gate**: điểm kiểm tra quyết định dữ liệu có được đi tiếp, quarantine hoặc dừng pipeline.; **ETL**: Extract–Transform–Load — lấy, biến đổi rồi nạp dữ liệu.; **ELT**: Extract–Load–Transform — lấy, nạp raw trước rồi biến đổi trong kho đích.
...
> **Đáp án:** D
> **Giải thích:** Không có row mới có thể trông như success; volume và freshness phải có threshold.
> **Thuật ngữ:** **threshold**: ngưỡng điểm dùng để loại kết quả có mức liên quan quá thấp.; **freshness**: độ mới của dữ liệu so với thời điểm user cần sử dụng.; **snapshot**: ảnh chụp dữ liệu tại một thời điểm, thường dùng để quét theo lịch.
...
> **Đáp án:** A
> **Giải thích:** CDC phải truyền và xử lý semantics của delete, không chỉ create/update.
> **Thuật ngữ:** **CDC**: Change Data Capture — ghi nhận các bản ghi được tạo, sửa hoặc xóa.
...
> **Đáp án:** B
> **Giải thích:** Data quality dimensions cần tách câu hỏi: không trùng, đúng kiểu/giá trị và đủ dữ liệu.
> **Thuật ngữ:** Không có thuật ngữ tiếng Anh chuyên môn mới cần giải thích trong câu này.
...
> **Đáp án:** C
> **Giải thích:** Drift chỉ báo phân phối lệch baseline; cần rule/domain knowledge để phán định.
> **Thuật ngữ:** **baseline**: mốc chuẩn dùng để so sánh với cách làm hiện tại.; **drift**: sự thay đổi phân phối dữ liệu hoặc hành vi so với mốc tham chiếu.
...
> **Đáp án:** D
> **Giải thích:** Day 10 nêu thứ tự Freshness → Volume → Schema → Lineage để khoanh tầng dữ liệu.
> **Thuật ngữ:** **freshness**: độ mới của dữ liệu so với thời điểm user cần sử dụng.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.
...
> **Đáp án:** A
> **Giải thích:** Natural key như (doc_id, version) giúp retry ghi đè đúng bản ghi thay vì tạo bản sao.
> **Thuật ngữ:** **idempotency**: tính lặp an toàn; chạy lại cùng request không tạo thêm side effect hoặc bản ghi trùng.
...
> **Đáp án:** C
> **Giải thích:** DLQ cô lập poison message; alert và replay có kiểm soát bảo vệ pipeline chính.
> **Thuật ngữ:** **DLQ**: Dead-Letter Queue — hàng đợi cách ly event lỗi.
...
> **Đáp án:** B
> **Giải thích:** Guardrails là lớp kiểm soát bắt buộc khi agent có dữ liệu, tool hoặc hành động thực tế.
> **Thuật ngữ:** **guardrails**: hàng rào kiểm soát; các lớp giới hạn input, model, output và hành động rủi ro.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.
...
> **Đáp án:** C
> **Giải thích:** Input rails có thể kiểm tra độ dài, format, topic và dấu hiệu prompt injection.
> **Thuật ngữ:** **prompt injection**: tấn công chèn instruction để thao túng hoặc ghi đè chỉ dẫn của agent.; **guardrails**: hàng rào kiểm soát; các lớp giới hạn input, model, output và hành động rủi ro.; **prompt**: chỉ dẫn hoặc đầu vào bằng ngôn ngữ tự nhiên gửi cho model.; **LLM**: Large Language Model — mô hình ngôn ngữ lớn.
...
> **Đáp án:** D
> **Giải thích:** Output rails là tuyến cuối để redact, block hoặc yêu cầu kiểm tra lại response.
> **Thuật ngữ:** **guardrails**: hàng rào kiểm soát; các lớp giới hạn input, model, output và hành động rủi ro.; **grounding**: neo câu trả lời vào evidence được cung cấp, không tự suy diễn ngoài nguồn.; **PII**: Personally Identifiable Information — dữ liệu có thể nhận diện một cá nhân.
...
> **Đáp án:** A
> **Giải thích:** Input rails, LLM rails, output rails và human review bổ sung cho nhau.
> **Thuật ngữ:** **defense in depth**: phòng thủ nhiều lớp; không phụ thuộc vào một bộ lọc duy nhất.; **LLM**: Large Language Model — mô hình ngôn ngữ lớn.; **AI**: Artificial Intelligence — trí tuệ nhân tạo.
...
> **Đáp án:** C
> **Giải thích:** Red team tấn công có chủ đích để kiểm tra guardrails trước khi người xấu làm.
> **Thuật ngữ:** **red teaming**: kiểm thử đối kháng bằng cách chủ động tấn công hệ thống để tìm lỗ hổng.; **guardrails**: hàng rào kiểm soát; các lớp giới hạn input, model, output và hành động rủi ro.; **prompt**: chỉ dẫn hoặc đầu vào bằng ngôn ngữ tự nhiên gửi cho model.
...
> **Đáp án:** D
> **Giải thích:** Human-in-the-loop thêm phê duyệt cho hành động khó hoàn tác hoặc high-stakes.
> **Thuật ngữ:** **HITL**: Human-in-the-Loop — con người duyệt hoặc can thiệp tại điểm rủi ro.
...
> **Đáp án:** A
> **Giải thích:** 12-Factor App tách config và secret khỏi code để dễ bảo vệ, thay đổi và triển khai.
> **Thuật ngữ:** **environment variables**: biến môi trường dùng để cấp config/secrets mà không hardcode vào code.; **API**: Application Programming Interface — giao diện để phần mềm gửi yêu cầu và nhận kết quả từ dịch vụ.
...
> **Đáp án:** B
> **Giải thích:** Liveness và readiness probe hỗ trợ restart hoặc loại instance lỗi khỏi traffic.
> **Thuật ngữ:** **health check**: endpoint/probe kiểm tra container còn sống hoặc sẵn sàng nhận request.
...
> **Đáp án:** D
> **Giải thích:** Day 13 bổ sung continuous eval như pillar thứ tư riêng cho AI.
...
> **Đáp án:** C
> **Giải thích:** Percentile cao cho thấy trải nghiệm của nhóm request chậm, thường quan trọng trong agent.
> **Thuật ngữ:** **latency**: độ trễ; thời gian từ lúc gửi yêu cầu đến khi nhận kết quả.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.; **P99**: percentile 99; cho thấy tail latency của nhóm request chậm nhất.
...
> **Đáp án:** B
> **Giải thích:** Chỉ dẫn độc hại nằm trực tiếp trong input user và cố ghi đè system prompt.
> **Thuật ngữ:** **direct prompt injection**: prompt injection nằm trực tiếp trong input của user.; **prompt injection**: tấn công chèn instruction để thao túng hoặc ghi đè chỉ dẫn của agent.; **system prompt**: chỉ dẫn nền do ứng dụng đặt để định nghĩa vai trò, luật và capability của model.; **prompt**: chỉ dẫn hoặc đầu vào bằng ngôn ngữ tự nhiên gửi cho model.
...
> **Đáp án:** C
> **Giải thích:** Instruction nằm trong nội dung mà agent retrieve, không nằm trực tiếp trong câu hỏi user.
> **Thuật ngữ:** **indirect prompt injection**: prompt injection ẩn trong tài liệu, web hoặc tool output mà agent đọc.; **prompt injection**: tấn công chèn instruction để thao túng hoặc ghi đè chỉ dẫn của agent.; **prompt**: chỉ dẫn hoặc đầu vào bằng ngôn ngữ tự nhiên gửi cho model.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.
...
> **Đáp án:** D
> **Giải thích:** Giới hạn quyền giảm blast radius nếu model hoặc tool output bị thao túng.
> **Thuật ngữ:** **least privilege**: nguyên tắc chỉ cấp đúng quyền tối thiểu cần cho agent hoặc service.; **sandbox**: môi trường cách ly để giới hạn quyền và tác động của code/tool.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.
...
> **Đáp án:** A
> **Giải thích:** Publish có side effect và rủi ro danh tiếng; nên phê duyệt trước khi thực hiện.
> **Thuật ngữ:** **side effect**: tác động bên ngoài như gửi email, xóa dữ liệu, trừ tiền hoặc đặt vé.; **HITL**: Human-in-the-Loop — con người duyệt hoặc can thiệp tại điểm rủi ro.
...
> **Đáp án:** B
> **Giải thích:** Builder chứa công cụ build; runtime image chỉ giữ dependency cần chạy.
> **Thuật ngữ:** **Docker**: công cụ đóng gói ứng dụng và dependency thành container có thể chạy nhất quán.
...
> **Đáp án:** C
> **Giải thích:** SSE báo tiến trình sớm; task dài nên chuyển queue/job để không giữ HTTP request quá lâu.
> **Thuật ngữ:** **streaming**: trả kết quả từng phần ngay khi sinh, thay vì chờ toàn bộ output.; **reasoning**: suy luận; phân tích mục tiêu và chọn bước tiếp theo.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.; **task**: nhiệm vụ cụ thể mà prompt yêu cầu model hoàn thành.; **UX**: User Experience — trải nghiệm người dùng khi tương tác với sản phẩm.
...
> **Đáp án:** D
> **Giải thích:** External state cho phép load balancer phân request sang nhiều instance.
> **Thuật ngữ:** **stateless**: không giữ state phiên trong instance; state thường được lưu ở hệ thống bên ngoài.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.
...
> **Đáp án:** A
> **Giải thích:** Gateway lọc quyền truy cập, tốc độ và ngân sách trước khi request chạm logic agent.
> **Thuật ngữ:** **API gateway**: cổng trước backend để xác thực, giới hạn tốc độ và bảo vệ chi phí.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.; **API**: Application Programming Interface — giao diện để phần mềm gửi yêu cầu và nhận kết quả từ dịch vụ.
...
> **Đáp án:** B
> **Giải thích:** Correlation ID giúp đi từ request đến LLM call, tool call và lỗi liên quan.
> **Thuật ngữ:** **LLM**: Large Language Model — mô hình ngôn ngữ lớn.
...
> **Đáp án:** D
> **Giải thích:** SLI là số đo; SLO là mục tiêu nội bộ; SLA là cam kết dịch vụ với hệ quả nếu vi phạm.
> **Thuật ngữ:** **SLI**: Service Level Indicator — chỉ số đo dịch vụ, ví dụ phần trăm request dưới 5 giây.; **SLO**: Service Level Objective — mục tiêu chất lượng/độ tin cậy cần đạt.; **SLA**: Service Level Agreement — cam kết dịch vụ với người dùng hoặc khách hàng.
...
> **Đáp án:** B
> **Giải thích:** Private data, untrusted content và external side effect kết hợp làm blast radius lớn.
> **Thuật ngữ:** **side effect**: tác động bên ngoài như gửi email, xóa dữ liệu, trừ tiền hoặc đặt vé.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.
...
> **Đáp án:** C
> **Giải thích:** Filter đơn lẻ dễ bị bypass; defense in depth và tách data/instruction xử lý nội dung retrieved.
> **Thuật ngữ:** **defense in depth**: phòng thủ nhiều lớp; không phụ thuộc vào một bộ lọc duy nhất.; **grounding**: neo câu trả lời vào evidence được cung cấp, không tự suy diễn ngoài nguồn.
...
> **Đáp án:** D
> **Giải thích:** Giới hạn action space và tách selector khỏi executor làm giảm quyền tự do nguy hiểm.
> **Thuật ngữ:** **action**: hành động được agent chọn để tiến tới mục tiêu.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.
...
> **Đáp án:** A
> **Giải thích:** Human-in-the-loop trước hành động không thể hoàn tác, cùng least privilege và audit log.
> **Thuật ngữ:** **least privilege**: nguyên tắc chỉ cấp đúng quyền tối thiểu cần cho agent hoặc service.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.; **HITL**: Human-in-the-Loop — con người duyệt hoặc can thiệp tại điểm rủi ro.
...
> **Đáp án:** B
> **Giải thích:** 12-Factor tách secret khỏi image; scan và secret manager giảm nguy cơ rò rỉ trong layer.
> **Thuật ngữ:** **Docker**: công cụ đóng gói ứng dụng và dependency thành container có thể chạy nhất quán.; **API**: Application Programming Interface — giao diện để phần mềm gửi yêu cầu và nhận kết quả từ dịch vụ.
...
> **Đáp án:** C
> **Giải thích:** Stateful process khó scale ngang; external session store giải quyết affinity và durability.
> **Thuật ngữ:** **stateless**: không giữ state phiên trong instance; state thường được lưu ở hệ thống bên ngoài.; **context**: bối cảnh hoặc dữ liệu nền được đưa cho model để xử lý task.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.
...
> **Đáp án:** D
> **Giải thích:** Idempotency key, durable execution hoặc saga giúp tránh lặp side effect.
> **Thuật ngữ:** **idempotency**: tính lặp an toàn; chạy lại cùng request không tạo thêm side effect hoặc bản ghi trùng.; **side effect**: tác động bên ngoài như gửi email, xóa dữ liệu, trừ tiền hoặc đặt vé.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.
...
> **Đáp án:** A
> **Giải thích:** Percentile cao cho thấy tail latency; agent nhiều lượt làm xác suất gặp một request chậm tăng.
> **Thuật ngữ:** **latency**: độ trễ; thời gian từ lúc gửi yêu cầu đến khi nhận kết quả.; **agent**: hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu.; **P50**: percentile 50; khoảng một nửa request nhanh hơn hoặc bằng mốc này.; **P99**: percentile 99; cho thấy tail latency của nhóm request chậm nhất.
...
> **Đáp án:** B
> **Giải thích:** Raw PII cần redact/allowlist; label tự do như request_id tạo quá nhiều time series.
> **Thuật ngữ:** **cardinality**: số lượng tổ hợp giá trị của label; quá cao làm metric tốn storage và khó vận hành.; **prompt**: chỉ dẫn hoặc đầu vào bằng ngôn ngữ tự nhiên gửi cho model.; **PII**: Personally Identifiable Information — dữ liệu có thể nhận diện một cá nhân.
...
> **Đáp án:** C
> **Giải thích:** Trace chỉ ra bottleneck retrieval; sửa hạ tầng/query và bổ sung eval/observability để tránh tái diễn.
...
Bảng này tổng hợp các thuật ngữ xuất hiện trong bộ câu hỏi để tiện tra cứu nhanh.

| Thuật ngữ | Giải thích tiếng Việt |
|---|---|
| **A2A** | Agent-to-Agent — giao tiếp giữa các agent qua message contract. |
| **ACL** | Access Control List — danh sách quyền truy cập gắn với tài nguyên hoặc dữ liệu. |
| **acting** | hành động; bước agent thực thi tool hoặc tác động lên môi trường. |
| **action** | hành động được agent chọn để tiến tới mục tiêu. |
| **actor** | tác nhân trực tiếp thực hiện hoặc chịu ảnh hưởng bởi workflow. |
| **agent** | hệ thống dùng model để lập kế hoạch, gọi tool, quan sát kết quả và lặp đến khi hoàn thành mục tiêu. |
| **AI** | Artificial Intelligence — trí tuệ nhân tạo. |
| **answer relevance** | mức độ câu trả lời đi đúng trọng tâm câu hỏi của user. |
| **API** | Application Programming Interface — giao diện để phần mềm gửi yêu cầu và nhận kết quả từ dịch vụ. |
| **API gateway** | cổng trước backend để xác thực, giới hạn tốc độ và bảo vệ chi phí. |
| **Artificial Intelligence** | AI — trí tuệ nhân tạo; lĩnh vực tạo hệ thống có khả năng thực hiện nhiệm vụ thường cần trí tuệ con người. |
| **augmentation** | AI hỗ trợ con người, còn người dùng giữ quyền quyết định cuối. |
| **automation** | AI tự thực hiện phần lớn hoặc toàn bộ workflow theo quy tắc đã định. |
| **autonomous** | tự chủ; có thể tự quyết định và thực hiện nhiều bước trong phạm vi được cấp. |
| **backpressure** | cơ chế làm chậm producer khi consumer hoặc downstream quá tải. |
| **baseline** | mốc chuẩn dùng để so sánh với cách làm hiện tại. |
| **batch** | xử lý dữ liệu theo lô vào thời điểm định trước. |
| **bi-encoder** | model encode query và document riêng để tìm kiếm nhanh bằng vector. |
| **BM25** | thuật toán sparse search chấm mức phù hợp của từ khóa trong tài liệu. |
| **cardinality** | số lượng tổ hợp giá trị của label; quá cao làm metric tốn storage và khó vận hành. |
| **CDC** | Change Data Capture — ghi nhận các bản ghi được tạo, sửa hoặc xóa. |
| **chain-of-thought** | suy luận theo chuỗi bước; dùng để hỗ trợ task cần nhiều bước reasoning. |
| **chatbot** | hệ thống hội thoại thường trả lời bằng văn bản theo context của lượt chat. |
| **citation** | trích dẫn nguồn; chỉ ra tài liệu hoặc dòng bằng chứng dùng cho câu trả lời. |
| **cold start** | thời gian khởi động instance mới trước khi sẵn sàng phục vụ. |
| **confidence** | mức độ tự tin/độ chắc chắn mà hệ thống gán cho output hoặc evidence. |
| **context** | bối cảnh hoặc dữ liệu nền được đưa cho model để xử lý task. |
| **context bleed** | lẫn ngữ cảnh; model nhầm dữ liệu không tin cậy thành instruction cần tuân theo. |
| **context recall** | mức độ retriever mang về đủ evidence cần thiết để trả lời. |
| **context window** | cửa sổ ngữ cảnh; tổng số token model có thể xử lý trong một lần gọi. |
| **cosine similarity** | độ tương đồng cosine; đo độ gần về hướng giữa hai vector. |
| **cross-encoder** | model đọc query và document cùng nhau để chấm relevance chính xác hơn. |
| **data contract** | thỏa thuận về schema, kiểu dữ liệu, giá trị hợp lệ và trách nhiệm giữa các bên. |
| **dead-letter queue** | DLQ — hàng đợi chứa message xử lý thất bại để điều tra hoặc replay. |
| **defense in depth** | phòng thủ nhiều lớp; không phụ thuộc vào một bộ lọc duy nhất. |
| **delimiter** | dấu phân cách dùng để tách instruction khỏi dữ liệu đầu vào. |
| **dense retrieval** | truy xuất dày đặc dựa trên vector ngữ nghĩa. |
| **direct prompt injection** | prompt injection nằm trực tiếp trong input của user. |
| **discriminative** | mô hình phân biệt; thường phân loại hoặc dự đoán nhãn đã định. |
| **DLQ** | Dead-Letter Queue — hàng đợi cách ly event lỗi. |
| **Docker** | công cụ đóng gói ứng dụng và dependency thành container có thể chạy nhất quán. |
| **DPO** | Direct Preference Optimization — tối ưu trực tiếp theo cặp câu trả lời được con người ưa thích. |
| **drift** | sự thay đổi phân phối dữ liệu hoặc hành vi so với mốc tham chiếu. |
| **dynamic prompt** | prompt được dựng theo trạng thái, ngày giờ, quyền hoặc availability hiện tại. |
| **ELT** | Extract–Load–Transform — lấy, nạp raw trước rồi biến đổi trong kho đích. |
| **embedding** | vector số biểu diễn ngữ nghĩa của văn bản, hình ảnh hoặc dữ liệu khác. |
| **enum** | tập hữu hạn các giá trị được phép chọn. |
| **environment variables** | biến môi trường dùng để cấp config/secrets mà không hardcode vào code. |
| **error budget** | ngân sách lỗi được phép theo SLO trước khi phải ưu tiên ổn định. |
| **ETL** | Extract–Transform–Load — lấy, biến đổi rồi nạp dữ liệu. |
| **eval** | viết tắt của evaluation — đánh giá chất lượng AI. |
| **evaluation** | đánh giá chất lượng hệ thống trên bộ case và tiêu chí đã định. |
| **exponential backoff** | tăng dần thời gian chờ giữa các lần retry, thường theo cấp số nhân. |
| **faithfulness** | mức độ câu trả lời bám trung thực vào context được truy xuất. |
| **false negative** | bỏ sót một trường hợp thực sự cần được phát hiện. |
| **false positive** | báo nhầm một trường hợp tốt/thấp rủi ro thành trường hợp dương tính. |
| **few-shot** | cung cấp một vài ví dụ để model bắt chước cách xử lý hoặc format. |
| **fine-tuning** | tinh chỉnh model bằng dữ liệu chuyên biệt để thay đổi hành vi hoặc phong cách. |
| **freshness** | độ mới của dữ liệu so với thời điểm user cần sử dụng. |
| **generation** | tạo sinh; bước LLM tổng hợp câu trả lời từ context đã truy xuất. |
| **generative** | mô hình tạo sinh; có khả năng tạo nội dung mới từ mẫu đã học. |
| **golden dataset** | bộ câu hỏi và đáp án/evidence chuẩn dùng làm mốc đánh giá. |
| **graceful degradation** | suy giảm có kiểm soát; khi thiếu dữ liệu/lỗi, hệ thống nói rõ và dùng fallback an toàn. |
| **grounding** | neo câu trả lời vào evidence được cung cấp, không tự suy diễn ngoài nguồn. |
| **guardrails** | hàng rào kiểm soát; các lớp giới hạn input, model, output và hành động rủi ro. |
| **health check** | endpoint/probe kiểm tra container còn sống hoặc sẵn sàng nhận request. |
| **HITL** | Human-in-the-Loop — con người duyệt hoặc can thiệp tại điểm rủi ro. |
| **hybrid search** | tìm kiếm lai kết hợp dense semantic search với sparse keyword search. |
| **HyDE** | Hypothetical Document Embeddings — tạo văn bản giả định để đưa query gần không gian tài liệu. |
| **idempotency** | tính lặp an toàn; chạy lại cùng request không tạo thêm side effect hoặc bản ghi trùng. |
| **indexing** | lập chỉ mục; parse, clean, chunk, embed và lưu tài liệu để tìm kiếm. |
| **indirect prompt injection** | prompt injection ẩn trong tài liệu, web hoặc tool output mà agent đọc. |
| **jitter** | độ ngẫu nhiên nhỏ cộng vào thời gian retry để tránh nhiều client retry cùng lúc. |
| **JSON schema** | schema JSON; mô tả cấu trúc, kiểu dữ liệu và field hợp lệ của JSON. |
| **LangGraph** | framework biểu diễn workflow agent bằng state, node, edge và conditional routing. |
| **Large Language Model** | LLM — mô hình ngôn ngữ lớn được huấn luyện trên lượng văn bản lớn. |
| **latency** | độ trễ; thời gian từ lúc gửi yêu cầu đến khi nhận kết quả. |
| **least privilege** | nguyên tắc chỉ cấp đúng quyền tối thiểu cần cho agent hoặc service. |
| **LLM** | Large Language Model — mô hình ngôn ngữ lớn. |
| **LLM-as-a-Judge** | dùng một LLM làm bộ chấm điểm cho output của hệ thống AI khác. |
| **logs** | bản ghi sự kiện chi tiết của hệ thống, thường dùng để tìm nguyên nhân lỗi. |
| **long-term memory** | bộ nhớ dài hạn; thông tin được lưu để truy xuất lại qua nhiều phiên. |
| **Machine Learning** | ML — học máy; nhánh AI học quy luật từ dữ liệu thay vì chỉ dùng luật viết sẵn. |
| **MCP** | Model Context Protocol — chuẩn kết nối agent với tools và resources bên ngoài. |
| **metadata** | siêu dữ liệu như nguồn, loại, ngày, khu vực hoặc quyền của một chunk. |
| **metrics** | số đo định lượng theo thời gian như latency, error rate, token hoặc cost. |
| **ML** | Machine Learning — học máy. |
| **MMR** | Maximum Marginal Relevance — cân bằng relevance với độ đa dạng, giảm chunk trùng lặp. |
| **multi-agent** | hệ nhiều agent được phân vai và phối hợp cho cùng một nhiệm vụ. |
| **multi-stage build** | build nhiều giai đoạn để image runtime chỉ giữ dependency cần thiết. |
| **MVP** | Minimum Viable Product — sản phẩm tối thiểu nhưng dùng được để kiểm chứng giả thuyết. |
| **next-token prediction** | dự đoán token tiếp theo dựa trên các token đã có. |
| **North Star metric** | chỉ số trung tâm gắn với outcome quan trọng nhất của sản phẩm. |
| **observability** | khả năng hiểu trạng thái nội bộ hệ thống từ metrics, logs, traces và eval. |
| **observation** | quan sát; kết quả tool hoặc môi trường trả lại cho agent. |
| **output contract** | hợp đồng đầu ra; quy định format, field, giới hạn và điều kiện trả kết quả. |
| **P50** | percentile 50; khoảng một nửa request nhanh hơn hoặc bằng mốc này. |
| **P95** | percentile 95; khoảng 95% request nhanh hơn hoặc bằng mốc này. |
| **P99** | percentile 99; cho thấy tail latency của nhóm request chậm nhất. |
| **pagination** | chia kết quả lớn thành nhiều trang hoặc batch nhỏ để đọc tuần tự. |
| **PII** | Personally Identifiable Information — dữ liệu có thể nhận diện một cá nhân. |
| **pre-training** | huấn luyện trước trên dữ liệu lớn để học ngôn ngữ và tri thức nền. |
| **precision** | tỷ lệ kết quả được dự đoán dương tính mà thực sự đúng. |
| **problem statement** | mô tả bài toán rõ actor, pain point, metric, boundary và cách đánh giá. |
| **prompt** | chỉ dẫn hoặc đầu vào bằng ngôn ngữ tự nhiên gửi cho model. |
| **prompt injection** | tấn công chèn instruction để thao túng hoặc ghi đè chỉ dẫn của agent. |
| **prototype** | bản mẫu nhanh để kiểm tra giả thuyết trước khi build đầy đủ. |
| **quality gate** | điểm kiểm tra quyết định dữ liệu có được đi tiếp, quarantine hoặc dừng pipeline. |
| **query decomposition** | chia câu hỏi phức tạp thành các sub-query nhỏ hơn. |
| **query expansion** | mở rộng query bằng từ đồng nghĩa, thuật ngữ hoặc biến thể để tăng recall. |
| **RACI** | ma trận Responsible, Accountable, Consulted, Informed để phân trách nhiệm. |
| **RAG** | Retrieval-Augmented Generation — tạo sinh tăng cường bằng truy xuất tài liệu. |
| **RAGAS** | framework/nhóm metric đánh giá RAG, thường gồm context recall, faithfulness và answer relevance. |
| **ReAct** | Reasoning + Acting — suy luận kết hợp hành động theo vòng Thought → Action → Observation. |
| **reasoning** | suy luận; phân tích mục tiêu và chọn bước tiếp theo. |
| **recall** | tỷ lệ các trường hợp đúng mà hệ thống tìm được. |
| **red teaming** | kiểm thử đối kháng bằng cách chủ động tấn công hệ thống để tìm lỗ hổng. |
| **required** | field bắt buộc phải có trong object hoặc tool arguments. |
| **reranking** | xếp hạng lại; chấm kỹ một tập candidate nhỏ sau bước retrieve rộng. |
| **retrieval** | truy xuất; tìm các chunk liên quan từ kho dữ liệu. |
| **RLHF** | Reinforcement Learning from Human Feedback — học tăng cường từ phản hồi của con người. |
| **role** | vai trò model cần đảm nhận trong prompt. |
| **RRF** | Reciprocal Rank Fusion — gộp kết quả dựa trên thứ hạng từ nhiều retriever. |
| **rule-based bot** | bot theo luật; phản hồi bằng các điều kiện if/else được định trước. |
| **sandbox** | môi trường cách ly để giới hạn quyền và tác động của code/tool. |
| **self-attention** | cơ chế để mỗi token cân nhắc mức liên quan với các token khác trong cùng context. |
| **semantic search** | tìm kiếm theo ý nghĩa thay vì chỉ khớp đúng từ khóa. |
| **SFT** | Supervised Fine-Tuning — tinh chỉnh có giám sát bằng các ví dụ input-output. |
| **shared state** | trạng thái dùng chung giữa các node/agent trong một workflow. |
| **short-term memory** | bộ nhớ ngắn hạn; lịch sử/context của task hiện tại. |
| **side effect** | tác động bên ngoài như gửi email, xóa dữ liệu, trừ tiền hoặc đặt vé. |
| **SLA** | Service Level Agreement — cam kết dịch vụ với người dùng hoặc khách hàng. |
| **SLI** | Service Level Indicator — chỉ số đo dịch vụ, ví dụ phần trăm request dưới 5 giây. |
| **SLO** | Service Level Objective — mục tiêu chất lượng/độ tin cậy cần đạt. |
| **snapshot** | ảnh chụp dữ liệu tại một thời điểm, thường dùng để quét theo lịch. |
| **sparse retrieval** | truy xuất thưa dựa trên khớp từ khóa, thường dùng inverted index. |
| **stakeholder** | bên liên quan hoặc người có lợi ích/quyền quyết định trong bài toán. |
| **stateless** | không giữ state phiên trong instance; state thường được lưu ở hệ thống bên ngoài. |
| **stop condition** | điều kiện dừng; tiêu chí cho biết agent đã đủ bằng chứng hoặc không thể đi tiếp. |
| **streaming** | trả kết quả từng phần ngay khi sinh, thay vì chờ toàn bộ output. |
| **supervisor** | agent điều phối; route task tới worker và tổng hợp kết quả. |
| **system prompt** | chỉ dẫn nền do ứng dụng đặt để định nghĩa vai trò, luật và capability của model. |
| **task** | nhiệm vụ cụ thể mà prompt yêu cầu model hoàn thành. |
| **temperature** | tham số sampling điều khiển mức đa dạng/ngẫu nhiên của output. |
| **threshold** | ngưỡng điểm dùng để loại kết quả có mức liên quan quá thấp. |
| **token** | đơn vị văn bản mà LLM trực tiếp xử lý và thường dùng để tính chi phí. |
| **tool calling** | gọi công cụ; cho phép model yêu cầu ứng dụng thực thi API, database hoặc hàm bên ngoài. |
| **top-k** | số lượng kết quả gần nhất được lấy về trong một lần retrieval. |
| **top_p** | tham số giới hạn tập token có xác suất tích lũy cao được dùng khi sampling. |
| **traces** | dấu vết end-to-end cho thấy một request đi qua các bước nào và mất bao lâu. |
| **Transformer** | kiến trúc mạng dùng attention để xử lý quan hệ giữa các token trong chuỗi. |
| **UX** | User Experience — trải nghiệm người dùng khi tương tác với sản phẩm. |
| **vector store** | kho lưu vector, nội dung gốc và metadata để tìm kiếm theo ngữ nghĩa. |
| **worker** | agent chuyên trách một năng lực hẹp như retrieve, gọi tool hoặc summarize. |
| **workflow** | quy trình gồm các bước và bàn giao để hoàn thành một công việc. |
| **zero-shot** | thực hiện task mà không được cung cấp ví dụ mẫu. |